...
- 表单模板必须是“模板主档 + 多版本”。
- 所有版本型 RDO（例如表单模板、批记录模板、工艺路线、产品配置、文档）遵守统一规则：主档只聚合版本，不存在当前版本或默认版本；多个版本可在有效时间窗重叠时同时生效。
- 版本状态由时间实时计算，不允许人工切换。列表仅展示“生效”和“失效”：处于 `[生效时间, 失效时间)` 的版本为生效，未到生效时间或已经失效的版本均显示失效且不可引用。
- 业务配置必须引用具体子版本，而非主档。引用对象新增版本不得自动修改既有配置、生产记录或审计快照；需要采用新版本时，用户须在依赖对象的新建/编辑版本中显式替换引用。
- 后续版本治理能力（依赖影响分析、复制升版、受控批量替换、执行快照）的架构边界见 `docs/architecture/rdo-version-governance.md`；任何实现不得引入自动升级、默认版本或隐式最新版本选择。
- 表单编码和版本号由用户填写，不自动生成。
