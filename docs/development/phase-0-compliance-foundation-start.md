...
OPEN/COMPLETED -> VOIDED（仅作废申请审批通过后；有活动表单流程的 SUBMITTED 记录暂不允许作废）
```

变更和作废审批使用独立申请对象及流程状态。审批中只锁定来源表单并显示申请标记，不把表单主状态改成审批中、修订中或作废审批中；变更通过后由新修订成为当前有效版本，旧修订只读保留。
P0 不为活动表单审批中的记录发起作废，因为当前流程底座未定义受控取消和原审批任务失效语义。
...
本阶段也不要把产品做成通用低代码表单平台。所有能力都必须服务于 DHR 证据、生产执行、表单审批、作业处理、DHR 汇总、放行审批、追溯或受控电子记录数据完整性。
...
16. 记录控制可创建变更/作废申请并显式选择已发布审批流程版本，验证新修订、并行汇合、标红对比、受限撤回、转办/重新分配及日志审计；活动表单作废边界见详细设计。
17. 关键逻辑有自动化测试。
...
15. 实现 record-control 的变更/作废申请、修订落地；与 workflow 对接并行审批、日志和干预，与表单模块对接差异标红，与 DHR 对接影响评估和重汇总。
16. 补充架构文档。
17. 补充测试。
