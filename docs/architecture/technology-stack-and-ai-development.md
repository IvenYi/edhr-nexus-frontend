...
- 可私有化部署；
- 未来可演进为 SaaS/专有云；
- 支持大量批次、表单、事务、DHR 汇总、审计、流程日志和归档数据；
- 后端稳定、主流、易维护；
...
- PostgreSQL 适合承载结构化 DHR、字段值、事务实例、流程日志、审计事件、签名、追溯查询和归档元数据。
...
process       工艺路线、制程配置
transactions  事务列表、事务流程配置、事务配置、事务实例
production    工单、批次、SN 基础查询、批次执行、返工执行、批次事务
...
因此，核心 eDHR 系统不需要为了 AI 改成 Python。Python 应作为工具层和未来 AI 服务层存在，而不是承载受控记录、表单实例、事务实例、签名、审计、DHR 汇总和放行等核心受控逻辑。
