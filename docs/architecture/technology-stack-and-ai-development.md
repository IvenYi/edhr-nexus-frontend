...
本项目是面向高监管制造和复杂质量记录场景的 ToB eDHR 软件，核心要求不是追新，而是：

- 可商用；
- 可私有化部署；
- 未来可演进为 SaaS/专有云；
- 支持大量批次、表单、作业、DHR 汇总、审计、流程日志和归档数据；
- 后端稳定、主流、易维护；
...
- Spring Batch 适合处理 DHR 归档、批量导入、审计归档、流程日志归档、完整性重算、DHR 汇总重算、追溯索引重建等大批量任务。
- Java 生态对私有化部署、国产企业 IT 环境、长期维护和人员招聘更友好。
- 对 eDHR 这种受控生产质量记录系统，事务一致性、审计追踪、签名、权限、流程日志和稳定性比快速原型更重要。
...
- PostgreSQL 适合承载结构化 DHR、字段值、作业实例、流程日志、审计事件、签名、追溯查询和归档元数据。
...
```text
模板结构：FormTemplateVersion 使用 JSONB 保存结构，但版本生效后不可变
流程结构：WorkflowVersion 使用 JSONB/关系表保存节点和边，发布后不可变
制程结构：ProcessVersion 使用关系表保存产品、路线、DHR、放行单、表单、SOP、权限和作业规则
作业结构：WorkVersion 使用 JSONB/关系表保存步骤、条件和表单步骤，发布后不可变
...
- Java/Spring Boot 在 ToB 受控生产质量系统的稳定性、组织接受度和工程生态上更适合做核心。
...
主智能体负责业务沟通、实现和基础测试；业务知识本体建模智能体负责结构化知识沉淀；质量验证智能体负责独立审查和验证。具体角色边界、交接格式和完成门禁以仓库根目录的 `AGENTS.md` 及 `docs/agents/` 下的角色契约为准。

执行方式为子智能体驱动：主智能体按功能切片派发专业子智能体，并审核和集成其产物；固定角色契约与 Git 知识资产负责跨任务、跨电脑和跨对话的连续性。

专业角色不是用来替代主智能体对最终交付的负责。主智能体必须自动整理已确认业务决策并派发知识沉淀，在实现完成后组织独立验证；用户不需要维护智能体工作流文件。

当重复工作同时具备稳定长期职责、清晰所有权、可独立验收、后续会反复出现，并且专业化能明显提升质量或效率时，主智能体必须主动向用户提出新增长期专业智能体。未经用户确认不得增加固定角色。

协作采用三级执行节奏：L0 快速调整由主智能体直接验证；L1 普通业务功能将连续调试合并为一个功能切片，本体沉淀与开发并行并在切片结束时执行一次针对性质量验证；L2 关键生产、合规、迁移和发布变更执行完整门禁。风险分级用于减少重复等待，不用于降低高风险功能的验收标准，具体规则见 `AGENTS.md`。
...
6. 实现 `signature` 电子签名模型。
7. 实现 `workflow` 流程定义、版本、任务和日志。
8. 实现 `master-data` 基础建模。
9. 实现 `templates` 表单/DHR/放行单模板。
10. 实现 `process` 工艺路线和制程配置。
11. 实现 `work` 作业定义和作业实例。
12. 实现 `production` 工单、批次和工序执行。
13. 实现 `records` 表单实例和字段值。
14. 实现 `dhr` DHR 实例、DHR 汇总和完整性检查雏形。
15. 实现 `release` 放行单和冻结。
16. 前端再接入登录、工作台、模板列表、制程配置、批次执行、DHR 汇总、审计查询等基础页面。
...
因此，核心 eDHR 系统不需要为了 AI 改成 Python。Python 应作为工具层和未来 AI 服务层存在，而不是承载受控记录、表单实例、作业实例、签名、审计、DHR 汇总和放行等核心受控逻辑。
