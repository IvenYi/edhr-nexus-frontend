# Codex 协作交付与 eDHR 本体规则作业运行架构

## 1. 文档目的

本文定义 eDHR 面向项目交付和二次开发的目标架构。目标不是让每个客户项目复制并修改一套产品代码，也不是在产品内增加一个 AI 交付工作台，而是在稳定的标准产品内核上，通过业务本体、结构化规则、可视化作业编排、标准动作和项目插件完成受控定制。团队使用 Codex 作为产品外部的开发与交付工具，并通过仓库化协作契约和知识资产保持不同开发者、不同 Codex 会话之间的理解和质量一致。
...
2. **编排化**：跨步骤、需等待、需人工处理或有条件分支的业务作业。
...
| 作业编排 | 接下来执行哪一步、等待谁、如何分支和结束 | 作业定义、版本、实例和节点状态 | 只负责调度 |
| 动作处理器 | 如何可靠地执行一个具体动作 | 标准动作、项目插件、外部连接器 | 是 |

规则运行引擎负责确定性判断，作业编排器负责有状态的流程推进，动作处理器负责数据库、表单、生产状态、审计和外部系统等副作用。三者不得合并为可以执行任意脚本的无边界运行环境。
...
工单、批次或 SN 开始执行时，应保存适用的产品、产品簇、制程版本、路线、DHR、表单、文档和规则集，并形成生产执行快照。生产作业不在产品或工序配置中手工引用；作业运行时根据生产上下文按作业适用规则自动匹配。创建作业实例时必须另外冻结作业模板引用、创建时的当前已发布作业流程版本引用、命中的作业适用规则快照和该已发布流程的作业流程配置快照。后续发布新规则或作业版本只能影响新创建的生产对象或作业实例，不得静默改变正在执行或已经完成的历史对象。
...
![Codex 协作交付与 eDHR 本体规则作业运行架构](ai-enabled-custom-delivery-and-transaction-runtime.png)
...
- **eDHR 产品配置与运行平面（产品内部）**：产品提供 RDO、规则、作业定义和可视化编排等受控业务能力；生产系统通过规则引擎、作业编排器和已注册处理器执行，不直接调用生成式 AI 作出生产决策。
...
3. 连续完成业务代码、页面、数据库、接口、作业定义和基础测试；
...
可视化作业编排器是 eDHR 产品内的业务配置能力，用于定义项目自定义作业。它不是 AI 交付工作台，也不是任意脚本编辑器，而是受控节点和执行契约的图形化作者工具；Codex 可以在交付过程中生成作业草稿或配置变更，供人员在产品内确认。
...
复杂并行、嵌套子作业、补偿编排、跨系统长事务、库存联动和复杂消息策略可在后续按真实项目需要扩展。底层定义需要保留版本和节点类型扩展能力，但第一阶段不提前实现未验证节点。
...
- 根据事件、生产上下文和适用规则自动匹配并启动作业实例，不读取产品或工序上的手工作业引用；
- 保存业务主体，并在实例创建时冻结作业模板、当前已发布流程版本、命中规则快照和流程配置快照；
- 管理当前节点、等待状态、重试、完成、关闭和中止；
- 推进表单、人工任务、条件和动作节点；
- 使用幂等键避免重复触发；
- 记录节点日志、作业主线和审计事件。

作业实例使用创建时冻结的作业模板、已发布流程版本、命中规则快照和流程配置快照。发布新版本或修改适用规则不能改变存量实例的节点图、适用依据和执行语义。当前这些运行时能力尚未实现，本节定义的是未来执行契约。

### 5.6 动作处理器与项目插件

动作处理器是作业步骤和规则结果可以调用的确定性能力。每个处理器必须具有稳定标识、参数契约、权限要求、幂等策略、审计语义、异常语义和测试证据。
...
```text
装配完工事件
  -> 规则引擎按作业适用规则自动匹配作业模板及其当前已发布流程
  -> 创建作业实例并冻结作业模板、已发布流程版本、命中规则和流程配置快照
  -> 表单节点创建检验表单
  -> 人工任务等待主管确认
  -> 条件节点读取结构化检验结论
      -> 合格：作业完成并允许后续生产
      -> 不合格：调用已注册返工处理器
  -> 返工完成事件恢复原作业或启动受控后续作业
  -> 写入作业主线、审计和 DHR 候选证据
```

这里 Codex 不参与生产时的合格判断；合格条件、数据来源、作业版本和处理器都是发布前已确认的确定性资产。
...
1. 建立作业定义、版本、节点、边、适用范围和触发配置模型；
2. 建立最小可用可视化作业编排器；
3. 建立规则判断、作业实例和节点状态运行内核；
4. 支持表单、人工任务、条件和注册动作节点；
5. 建立标准动作注册和项目插件接口；
6. 建立草稿校验、模拟测试、人工确认和版本发布；
7. 在批次/SN工序工作台提供作业处理入口；
8. 创建作业实例时冻结作业模板、当前已发布流程版本、命中规则快照、流程配置快照和相关 RDO 版本引用；
9. 写入作业主线、节点日志、审计和 DHR 候选证据；
10. 让 Codex 能根据 `decisionPackage` 生成作业草稿、测试场景和项目插件骨架。
...
早期冠骋调研曾建议生产 P0 只预留作业上下文、到 P1 再实现最小作业闭环。当前产品决策已经调整：为了支撑 Codex 辅助的定制交付，**第一阶段即纳入最小可用的可视化自定义作业编排、作业运行时和动作扩展接口**。这项调整取代早期文档中的阶段划分建议，但不改变“暂不复制冠骋全部复杂节点”的范围控制。
...
1. Codex 生成的作业草稿在发布前不能创建生产作业实例。
2. 适用事件发生时按作业适用规则自动匹配有当前已发布流程的作业，不依赖产品或工序手工引用，并创建使用冻结快照的实例。
3. 动作节点只能调用注册且契约匹配的处理器。
4. 重复业务事件不会重复创建作业、表单或业务结果。
5. 作业新版本发布或适用规则修改后，存量实例继续使用原作业模板、已发布流程版本、命中规则快照和流程配置快照。
...
10. 团队成员可以借助 Codex、可视化作业流程和模拟数据完成常规流程确认，但高风险变更仍有明确审查门禁。

## 12. 与现有架构的关系

- 本体状态、投影和执行契约原则沿用[业务知识本体模型](business-knowledge-model.md)。
- 作业、投影、追溯和 DHR 证据边界沿用[作业、投影、追溯与 DHR 汇总业务边界](transaction-traceability-dhr-business-rules.md)。
...
- 冠骋事务能力仅作为业务参考，详见[冠骋事务模块调研](../reference-assets/research/crown-transaction-module-research.md)；本产品以“作业”作为对应业务术语，不复制其低代码平台实现。
