...
- 普通字段在分组内使用 `src/components/FormDialogFieldGrid.tsx`：桌面双列、窄屏单列；字段数量为奇数时，最后一个普通字段自动跨满一行。若最后一个子节点已经是长文本、上传等显式跨列字段，使用 `hasTrailingFullRow` 让网格按倒数第二个普通字段的奇偶性布局。长文本、上传等字段可设置 `sx={{ gridColumn: { sm: '1 / -1' } }}` 跨两列；不要用页面级 `Stack` 让整张普通表单退成单列。
