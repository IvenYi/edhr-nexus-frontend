...
截至 2026-09-19 的当前切片状态：DHR列表 与 DHR汇总 已按独立菜单实现；汇总草稿、多级覆盖目录、三类表单实例候选、不可变汇总版本、来源快照/哈希、独立权限以及产品/产品簇制程版本的 `DHR_SUMMARY` 审核策略绑定已有 working-tree 原始实现。DHR审核 独立菜单与审批运行时尚未实现，不能由 `PENDING_REVIEW` 状态推导为已有待办、签名、退回或批准能力。
...
### 阶段 2：DHR 汇总草稿和候选证据（working tree 已实现，待发布级验收）
...
- 左侧：DHR 模板目录树；
- 中间：当前目录项已归集的证据；
- 右侧：按当前真实来源契约分为作业表单和生产执行自定义表单候选；目录直接绑定实例自动进入基础目录项。

当前交互：

- 下拉选择目标目录：当前可访问且明确的归档操作；
- 查看实例：使用系统统一表单画布查看每一份真实实例；
- 拖拽和批量加入可作为后续交互增强，但不得改变底层受控关联语义。

底层数据模型必须是“来源记录关联到 DHR 汇总草稿版本的目录项”，不能复制或移动原表单。

前置条件：每一种候选来源必须有明确的来源 ID、所属生产对象、有效状态、版本、权限和审计字段。没有真实来源契约的模块不能只做一个空分类。

### 阶段 3：DHR 汇总流程配置和审批（配置已实现，审批运行时待后续）
