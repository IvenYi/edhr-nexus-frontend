...
业务沟通与确认
  -> 主智能体自动生成业务决策包
  -> 主智能体按 L0/L1/L2 生成相称的最小影响分析和扩展路径
  -> 本体建模智能体加载当前知识基线
  -> 本体建模智能体更新并校验知识资产
  -> 主智能体开发业务功能并完成基础测试
  -> 质量验证智能体独立审查和测试
...
需求影响分析由主智能体负责，不新增独立长期角色或独立审批流程。L0 只记录不适用依据；L1 在 decisionPackage 内维护直接影响、主要相邻风险、证据、测试范围和扩展路径；L2 才完整覆盖传递影响、潜在影响、不受影响、兼容迁移、快照、审计、权限和测试。分析只需达到当前风险判断所需的深度，不得阻塞无依赖实现。
