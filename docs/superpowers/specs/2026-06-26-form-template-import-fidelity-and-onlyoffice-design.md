...
在数据模块的模板建模 / 表单模板设计器中，导入 Excel、Word、PDF 后保留原始文件高保真样式，自动识别页面横竖方向，解析文档文字和结构，生成可确认的候选组件，再由用户确认哪些内容成为后续填报组件、哪些保留为静态文字。OnlyOffice 作为原始文档高保真编辑和渲染底座，Zencas 继续负责结构化字段层、拖拽设计、填报数据、权限、审计和版本发布。
...
- 不把 OnlyOffice 当成 Zencas 结构化字段模型的唯一存储。
...
- 原文高保真由 OnlyOffice 或文档转换服务承担，Zencas 只做结构化覆盖层和业务管控。
...
- 保存设计时只保存结构化覆盖层，不把填报值写进模板设计 JSON。
...
- Zencas 画布展示结构化覆盖层，覆盖层坐标与源文档页坐标对齐。
...
{
  "schemaVersion": "1.1",
  "strategy": "OnlyOffice高保真原文层+Zencas结构化覆盖层",
  "orientation": "landscape",
  "source": {
    "fileId": "301",
    "fileName": "生产巡检记录.xlsx",
