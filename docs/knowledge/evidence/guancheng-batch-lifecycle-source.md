...
- 工单拆分（`MfgOrderSplitContainerBs`）为新批次设置产品、产品簇、制程版本、工艺路线、首个工序、DHR 模板和放行模板引用，状态初始化为 `waiting`，创建打印记录，写入 `CONTAINER_WORK_CREATE` 事务及主题追溯关系，并回写工单的已创建批次数、已创建批次数量和未生产数量。该路径实际只按累计数量校验剩余量，没有生效的工单状态拦截；SN 拆分中曾有的工单状态拦截代码目前被注释。这不是既有批次行上的再拆分接口。
