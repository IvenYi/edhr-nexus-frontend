...
当前 `0.3.24` 同一切片于 2026-09-25 原位修订 `DEC-0063/0068/0071/0072`：生产对象的目录、作业、自定义实际实例及受控附件持续自动归集，不以手选或完成状态决定证据可见。普通汇总整理/提交仍限生产正常完成，终止政策保持 `DEC-0069`。未完成、失败、作废、替代及处置历史保留，表单 COMPLETED 不代表检验合格；必填目录只是适用核查的一项，未完成补录等阻断继续适用。目录、顺序与别名只是展示，整组或单份整理不缩减证据；取消自定义位置恢复默认位置。

提交重新核查完整集合及来源版本，要求提交人逐项人工确认质量与异常、源审批签署和完整范围并填写说明；确认人、时间、说明随 `checkResult` 冻结，不能称作系统自动判读任意质量结论。全部证据、附件、检查结果、展示目录和审批绑定同版冻结；审批任务附件下载按任务授权且只限该版附件，审批、详情与 ZIP 使用同一版本。ZIP 补源生产执行审计及签名，部分 ZIP 不冒充完整 DHR，暂不打印。NONE 不豁免源签署、法定审核或放行。来源生效变化自动检测提示并阻断过时批准，人工退回或新版本重审，旧批准及快照保留；实例日志和 DHR 审计分层。作业/工序表单由执行上下文绑定生产对象，内容填错、自定义上下文选错、附件误关联分别处理，不设计普通跨批次搬运。

当前工作树已有全部实际表单实例默认位置、逐实例冻结、受控附件快照、来源范围重核、审批时新增实例/附件比较、任务专属附件下载和 ZIP 导出源码及聚焦测试；旧手选/完成状态过滤只作为 Git 历史解释。`DhrReviewService` 已有任务、签署、批准和人工退回路径，不能笼统称审批未实现。submit 已校验三项人工确认及说明，`checkResult.manualReview` 保存确认人/时间/说明；ZIP 清单已有提交时点前来源生产执行审计与签名。当前客观检查主要覆盖必填目录、必需表单完成、未完成补录和附件核验；任意质量结论、异常处置及源签署有效性无通用自动判读，主动影响通知、外部审计全链及真实浏览器/数据库发布验收也未闭环。因此完整新设计仍保持 `specified/internal`，不变更 0.3.24 或 schema，不回写旧冻结版本，不建立新执行契约。旧汇总历史数据非本轮兼容或回填目标，无需处理且不主动删除。详情见四项原决策的 `implementationDiscrepancies`、现有确认证据及主设计第 6～11 节。中国仅采用 2025 新版设计基线，US/EU 为工程建议，市场与 SOP 缺口不阻塞通用设计，也不构成全集合规承诺。
...
审批语义的客户展示术语由 `DEC-0056` 统一：表达审批决策动作时页面、菜单、任务和流程配置使用“审批”；流程中心菜单/页面名称使用四字“审批流程”，不把“模板”放入该展示名。检查、评审、审计和 CAPA 的纠正措施等非审批语义保持原词；`REVIEW`、`APPROVAL`、`APPROVE`、路由、权限码和数据库标识保持不变。当前 DhrReviewService 已有审批运行路径；术语统一不证明完整证据新设计已实现或已交付。历史决策、历史审计记录及原始证据中的旧词仅为追溯保留，不构成现行产品展示名。
