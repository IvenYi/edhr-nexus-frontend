    ...
    void migrationPreservesLegacyNullTypesAndJpaPersistsOptionalBrand() throws Exception {
        ...
        try (var sessions = new Configuration()
            ...
            try (var connection = DriverManager.getConnection(url, "sa", ""); var sql = connection.createStatement()) {
                sql.execute("ALTER TABLE material DROP COLUMN brand");
            ...
            try (var session = sessions.openSession()) {
                ...
                assertThat(legacy.getBrand()).isNull();
                var transaction = session.beginTransaction();
                session.persist(Material.builder().id(2L).code("BRAND").name("带品牌物料").materialTypeId(3L).brand("示例品牌").build());
            ...
            try (var session = sessions.openSession()) {
                ...
                assertThat(branded.getBrand()).isEqualTo("示例品牌");
                assertThat(session.find(Material.class, 3L).getBrand()).isNull();
                var transaction = session.beginTransaction();
                branded.setBrand(null);
                transaction.commit();
            }
            try (var session = sessions.openSession()) {
                assertThat(session.find(Material.class, 2L).getBrand()).isNull();
            }
            try (var connection = DriverManager.getConnection(url, "sa", ""); var sql = connection.createStatement()) {
