...
mustInclude('MoreVertRounded', 'user operation pill should use the vertical more icon from the reference');
mustNotInclude('KeyboardArrowDownRounded', 'user operation pill should not use the old dropdown arrow');
mustInclude('data-app-user-pill', 'user operation button should expose a QA selector');
mustInclude('data-app-user-menu', 'user dropdown menu should expose a QA selector');
mustInclude("const userDisplayName = user.displayName || user.username || '管理员';", 'user operation area should prefer the Chinese display name over the account name');
mustInclude("const userInitial = userDisplayName.trim().charAt(0) || '管';", 'user operation avatar should derive the first visible character from the display name');
mustInclude("borderRadius: '999px'", 'user operation button should render as a full pill');
mustInclude("bgcolor: '#dbeafe'", 'user operation pill should use the light blue reference background');
mustInclude("bgcolor: '#4092ff'", 'user operation avatar should use the blue reference circle');
mustInclude('height: 34', 'user operation pill should use a smaller 34px height');
mustInclude('width: 26, height: 26', 'user operation avatar should shrink with the smaller pill');
mustInclude('fontSize: 14', 'user operation name should use a compact font size');
mustInclude('maxWidth: 72', 'user operation name should narrow the trigger width');
mustInclude('fontSize: 18, color: \'inherit\'', 'user operation more icon should scale down with the smaller pill');
mustInclude("borderRadius: '18px'", 'user dropdown menu should use the large rounded reference panel');
mustInclude('minWidth: 236', 'user dropdown menu should stay readable while matching the compact user operation area');
mustInclude('minHeight: 36', 'user dropdown rows should be compact after reducing the trigger height');
mustInclude('SwapHorizRounded', 'profile header should include the switch-account style icon');
mustInclude('userRoleText', 'user dropdown should render the current user role under the name');
mustInclude("import { getRoles } from '@/api/identity';", 'app shell should load role dictionary for cached users that only have role ids');
mustInclude("queryKey: ['roles-all']", 'app shell should reuse the shared role dictionary query key');
mustInclude('roleNameById', 'app shell should map role ids to Chinese role names');
mustInclude('岗位角色未映射', 'app shell should avoid showing raw long role ids when the dictionary is unavailable');
mustInclude('isRoleIdText', 'app shell should filter any cached numeric role-name strings before rendering');
mustNotInclude("user.roleIds?.filter(Boolean).join('、')", 'user dropdown should not display raw role id numbers directly');
mustInclude('个人设置', 'user dropdown should include only the personal settings action above logout');
mustInclude('PersonOutlineRounded', 'personal settings row should use the user/profile icon');
mustNotInclude('SentimentSatisfiedAltRounded', 'user dropdown should not keep the old status action');
mustNotInclude('Inventory2Outlined', 'user dropdown should remove the repository action');
mustNotInclude('StarBorderRounded', 'user dropdown should remove the stars action');
mustNotInclude('CodeRounded', 'user dropdown should remove the gists action');
mustNotInclude('ApartmentRounded', 'user dropdown should remove the organization action');
mustNotInclude('PublicRounded', 'user dropdown should remove the enterprise action');
mustNotInclude('FavoriteBorderRounded', 'user dropdown should remove the sponsors action');
mustNotInclude('GroupsOutlined', 'user dropdown should remove the copilot settings action');
mustNotInclude('ScienceOutlined', 'user dropdown should remove the feature preview action');
mustNotInclude('BrushOutlined', 'user dropdown should remove the appearance action');
mustNotInclude('AccessibilityNewRounded', 'user dropdown should remove the accessibility action');
mustNotInclude('FileUploadOutlined', 'user dropdown should remove the enterprise trial action');
mustNotInclude('设置状态', 'user dropdown should not show status settings after the annotated cleanup');
mustNotInclude('轮廓', 'user dropdown should remove the profile row from the previous reference');
mustNotInclude('存储库', 'user dropdown should remove the repository row');
mustNotInclude('星星', 'user dropdown should remove the stars row');
mustNotInclude('要点', 'user dropdown should remove the gists row');
mustNotInclude('组织', 'user dropdown should remove the organizations row');
mustNotInclude('企业', 'user dropdown should remove the enterprise row');
mustNotInclude('赞助商', 'user dropdown should remove the sponsors row');
mustNotInclude('副驾驶设置', 'user dropdown should remove copilot settings');
mustNotInclude('功能预览', 'user dropdown should remove feature preview');
mustNotInclude('外貌', 'user dropdown should remove appearance');
mustNotInclude('无障碍', 'user dropdown should remove accessibility');
mustNotInclude('试用企业版', 'user dropdown should remove enterprise trial');
mustNotInclude('自由的', 'user dropdown should remove the trial pill');
mustInclude('登出', 'user dropdown logout text should match the reference');
mustNotInclude('退出登录', 'user dropdown should not use the old logout label');
