...
mustInclude('tableScrollbarWidth', 'personnel table action shadow should account for the vertical scrollbar gutter');
...
mustNotInclude("position: 'relative',\n                          userSelect: 'none',", 'personnel table header cells should not override stickyHeader with relative positioning');
...
mustInclude('AuditFieldBlock', 'personnel detail audit records should render field-name blocks in expanded content');
...
mustInclude('data-audit-field-before', 'expanded audit record should expose before field content');
mustInclude('data-audit-field-after', 'expanded audit record should expose after field content');
...
mustInclude('AuditFieldBlock title="变更前"', 'expanded audit record should show before field content');
mustInclude('AuditFieldBlock title="变更后"', 'expanded audit record should show after field content');
mustInclude('auditFieldLabelMap', 'audit detail should map backend field keys to customer-facing names');
mustInclude("email: '邮箱'", 'audit detail should show the email field with a Chinese label');
mustInclude("phone: '手机号'", 'audit detail should show the phone field with a Chinese label');
mustInclude('formatAuditFieldRows', 'audit detail should normalize audit content into one row per field');
mustInclude('getAuditDisplayValue', 'audit detail should format empty values as a dash');
mustInclude('preserveAuditJsonLargeNumbers', 'audit detail should preserve large numeric organization ids before JSON parsing');
mustInclude('JSON.parse(preserveAuditJsonLargeNumbers(trimmed))', 'audit detail should parse audit JSON after preserving large ids as strings');
...
mustInclude('record.beforeFields', 'expanded audit record should render normalized before fields');
mustInclude('record.afterFields', 'expanded audit record should render normalized after fields');
mustNotInclude('AuditJsonBlock', 'personnel detail audit records should no longer render JSON blocks');
mustNotInclude('data-audit-json-before', 'expanded audit record should no longer expose before JSON content');
mustNotInclude('data-audit-json-after', 'expanded audit record should no longer expose after JSON content');
mustNotInclude("JSON.stringify(normalized, null, 2)", 'audit detail content should no longer be formatted as raw JSON');
mustNotInclude('component="pre"', 'audit detail content should no longer render raw JSON in a pre block');
...
mustInclude('height: 40', 'user dialog controls should follow the compact eDHR user-management height');
mustInclude('<DialogContent dividers>', 'user dialog should follow the eDHR divided content pattern');
...
mustInclude('size="small" sx={userSelectSx}', 'user dialog select controls should use the compact eDHR size');
mustInclude('<Table stickyHeader size="small"', 'personnel table should use compact density with sticky headers');
mustInclude('size="small"', 'personnel add action and user dialog should use compact eDHR sizing');
...
mustInclude('togglePersonnelSelection', 'personnel row checkbox should toggle controlled row selection');
mustInclude('togglePagePersonnelSelection', 'personnel header checkbox should toggle the current page rows');
mustInclude('checked={selectedPersonnelIds.has(row.id)}', 'personnel row checkbox should render checked state');
mustInclude('onChange={(event) => togglePersonnelSelection(row.id, event.target.checked)}', 'personnel row checkbox should update selection without opening details');
mustInclude('onClick={(event) => {\n            event.stopPropagation();\n            togglePersonnelSelection(row.id);\n          }}', 'clicking the selection cell should toggle selection instead of opening the row detail');
mustInclude('checked={allPagedRowsSelected}', 'personnel header checkbox should show all-page selected state');
mustInclude('indeterminate={isPagedRowsPartiallySelected}', 'personnel header checkbox should show partial-page selected state');
mustInclude('onChange={(event) => togglePagePersonnelSelection(event.target.checked)}', 'personnel header checkbox should select or clear current page rows');
