...
const productModelingPageContent = readFileSync(new URL('../src/pages/master-data/ProductModelingPage.tsx', import.meta.url), 'utf8');
const productProcessEditorContent = readFileSync(new URL('../src/pages/master-data/components/ProductProcessVersionEditorDialog.tsx', import.meta.url), 'utf8');
const productModelingWorkspaceContent = readFileSync(new URL('../src/pages/master-data/ProductModelingWorkspacePage.tsx', import.meta.url), 'utf8');
...
const productProcessControllerContent = readFileSync(new URL('../../backend/src/main/java/com/zencas/edhr/masterdata/controller/ProductProcessController.java', import.meta.url), 'utf8');
...
mustInclude(documentPageContent, "{ id: 'actions', label: '操作', defaultWidth: 128, minWidth: 128 }", 'document parent and version actions should reserve the shared three-action width');
mustInclude(documentPageContent, 'const sharedTableMinWidth = Math.max(mainTableBaseWidth, versionTableBaseWidth);', 'document parent and version tables should share a minimum table width');
mustNotInclude(documentPageContent, "{ id: 'code', label: '文档编码'", 'document master rows should not expose a parent-level code');
mustInclude(documentPageContent, "{ id: 'code', label: '编码', defaultWidth: 150", 'document child versions should expose the version code next to the version number');
mustInclude(documentPageContent, "case 'code': return <TableCell key={column.id}>{version.code}</TableCell>;", 'document child tables should display the code stored on the concrete version');
mustNotInclude(documentPageContent, 'document.code', 'document management should not render or edit a master-level code');
mustInclude(documentPageContent, 'label="文档编码" value={form.code}', 'document version dialogs should edit the version code');
mustNotInclude(documentPageContent, 'data-document-version-action-spacer', 'the shared RDO shell should align document fixed actions without spacer columns');
...
mustInclude(documentPageContent, "getOperationColumnSx(getResolvedColumnWidth(column, 'version'), 'head')", 'document version actions should retain their fixed standard width');
...
mustInclude(changelogMasterContent, '0052-product-process-document-pages.sql', 'operation document page ranges should have a database migration');

mustInclude(productProcessEditorContent, 'export function RdoVersionMultiSelect', 'product RDO references should use the shared hierarchical multi-select');
mustInclude(productProcessEditorContent, 'export function ReferenceBindingList', 'product RDO references should render as configurable binding rows');
mustInclude(productProcessEditorContent, '搜索分类、父名称、版本号或版本编码', 'product RDO reference selection should support category, parent, version, and code search');
mustInclude(productProcessEditorContent, '展示起始页', 'SOP references should expose a start page configuration');
mustInclude(productProcessEditorContent, '展示结束页', 'SOP references should expose an end page configuration');
mustInclude(productProcessEditorContent, 'pageStart', 'SOP page ranges should be part of the persisted operation binding payload');
mustInclude(productProcessEditorContent, "const DEFAULT_PRODUCTION_MODE = '量产';", 'new product process versions should use the fixed production mode');
mustInclude(productProcessEditorContent, "productionMode: mode === 'copy' ? DEFAULT_PRODUCTION_MODE : target.productionMode", 'copied product process versions should reset to the fixed production mode while edits preserve the stored field');
mustInclude(productProcessEditorContent, 'productionMode: DEFAULT_PRODUCTION_MODE,', 'new product process versions should initialize the hidden backend field');
mustInclude(productProcessEditorContent, 'productionMode: form.productionMode.trim(),', 'the hidden production mode should remain in product process payloads');
mustNotInclude(productProcessEditorContent, '<TextField select required size="small" label="生产模式"', 'the product process editor should hide production mode selection');
mustInclude(productProcessEditorContent, "const PRODUCTION_FORM_OPTIONS = ['批次', 'SN'];", 'the shared product process editor should only offer batch and SN production forms');
mustNotInclude(productProcessEditorContent, "const PRODUCTION_FORM_OPTIONS = ['批次', 'SN', '批次转SN'];", 'the shared product process editor should not offer batch-to-SN');
mustInclude(productProcessEditorContent, 'productionForm: normalizeProductionForm(target.productionForm),', 'the shared editor should force legacy unsupported values to be reselected');
mustInclude(productModelingWorkspaceContent, '<ReferenceBindingList', 'the product design page should use the same RDO reference rows');
mustNotInclude(productModelingWorkspaceContent, '<DocumentReferenceDialog', 'the product design page should not use the legacy document reference dialog');
mustInclude(productModelingWorkspaceContent, "const DEFAULT_PRODUCTION_MODE = '量产';", 'the legacy product design page should share the fixed production mode');
mustInclude(productModelingWorkspaceContent, 'productionMode: DEFAULT_PRODUCTION_MODE,', 'the legacy product design page should initialize new and copied records with the hidden backend field');
mustInclude(productModelingWorkspaceContent, 'productionMode: form.productionMode.trim(),', 'the legacy product design page should keep the hidden production mode in payloads');
mustNotInclude(productModelingWorkspaceContent, '<TextField select required size="small" label="生产模式"', 'the legacy product design page should hide production mode selection');
mustInclude(productModelingWorkspaceContent, "const PRODUCTION_MODALITY_OPTIONS = ['批次', 'SN'];", 'the legacy product design page should only offer batch and SN production forms');
mustNotInclude(productModelingWorkspaceContent, "const PRODUCTION_MODALITY_OPTIONS = ['批次', 'SN', '批次转SN'];", 'the legacy product design page should not offer batch-to-SN');
mustInclude(productModelingWorkspaceContent, 'productionForm: normalizeProductionForm(version.productionForm),', 'the legacy product design page should force legacy unsupported values to be reselected');
mustInclude(productModelingPageContent, 'aria-label="产品制程配置版本列表"', 'product versions should use the standard unframed RDO child table');
mustInclude(productModelingPageContent, 'const ACTION_COLUMN_WIDTH = 128;', 'product parent and version tables should share the standard action-column width');
mustInclude(productModelingPageContent, "{ id: 'actions', label: '操作', width: ACTION_COLUMN_WIDTH, minWidth: ACTION_COLUMN_WIDTH }", 'product version actions should reserve the standard three-action width');
mustNotInclude(productModelingPageContent, '└', 'product version rows should not render a relation marker before the version number');
mustInclude(productModelingPageContent, '<Edit fontSize="small" />', 'product version actions should expose the shared edit icon');
mustInclude(productModelingPageContent, '<ContentCopy fontSize="small" />', 'product version actions should expose the shared copy icon');
mustInclude(productModelingPageContent, '<Delete fontSize="small" />', 'product version actions should expose the shared delete icon');
mustNotInclude(productModelingPageContent, 'DesignServicesOutlined', 'product versions should be configured through edit instead of a separate design action');
mustNotInclude(productModelingPageContent, 'HistoryOutlined', 'product audit should remain in the version detail tab instead of the row action bar');
mustInclude(productModelingPageContent, '<Tab label="数据审计" />', 'product detail should retain the data-audit tab');
mustInclude(productModelingPageContent, "确定删除产品「${deleteTarget?.product.name || ''}」的制程版本", 'product version delete confirmation should identify the parent product');
mustInclude(productProcessControllerContent, 'operationFormBindingRepository.flush();', 'product version replacement should flush deleted form bindings before recreation');
mustInclude(productProcessControllerContent, 'operationDocumentBindingRepository.flush();', 'product version replacement should flush deleted document bindings before recreation');
mustInclude(productProcessControllerContent, 'operationBindingRepository.flush();', 'product version replacement should flush deleted operation bindings before recreation');
mustInclude(productProcessControllerContent, '"编辑工序配置"', 'form and document reference changes should be audited as edits');
mustInclude(productModelingPageContent, "if (value === 'ACTIVE') return '生效';", 'product audit should translate historical raw ACTIVE status values');
...
mustInclude(pageContent, '<TableRow data-record-id={route.id} key={route.id} hover onClick={() => expandRouteGroup(route.id)}', 'route parent row click should expand or collapse child rows');
...
mustInclude(pageContent, 'const PROCESS_ACTION_COLUMN_WIDTH = 128;', 'process modeling action column should keep the shared fixed width');
...
mustInclude(pageContent, "const sharedTableMinWidth = Math.max(totalTableWidth, pageKey === 'materials' ? totalMaterialVersionTableWidth : pageKey === 'routes' ? totalRouteVersionTableWidth : totalTableWidth);", 'process modeling parent and child tables should share a minimum table width');
mustInclude(pageContent, 'ListTableShell', 'process modeling parent and child tables should use the shared list-table shell');
mustInclude(pageContent, 'resolveListColumnWidths', 'process modeling parent and child tables should resolve widths through the shared list-table shell');
mustNotInclude(pageContent, 'data-process-main-action-spacer', 'the shared RDO shell should align process fixed actions without spacer columns');
...
mustInclude(pageContent, '...listTableStickyEdgeSx', 'process modeling action column should reuse the shared fixed-column divider shadow');
...
mustInclude(pageContent, 'resolveColumnWidths(materialVersionColumnWidths, 0, visibleMaterialVersionColumns)', 'material version base widths should come from persisted child-column widths');
...
mustInclude(pageContent, "label: id === 'version' ? '工艺路线版本号' : id === 'code' ? '编码' : id === 'status' ? '版本状态' : id === 'effectiveDate' ? '生效日期' : id === 'expiryDate' ? '失效日期' : id === 'description' ? '版本说明'", 'route version child table should expose version code and description');
...
mustInclude(pageContent, "const ROUTE_VERSION_FIELD_IDS: Array<keyof ProcessModelingPayload> = ['version', 'code', 'effectiveDate', 'expiryDate', 'versionDescription'];", 'route dialog should split version information fields');
mustInclude(pageContent, "(['version', 'code', 'status', 'effectiveDate'", 'route version code should be ordered immediately after the version label');
mustInclude(pageContent, 'code: version.code ?? \'\'', 'route version edit dialogs should preload the existing version code');
mustInclude(pageContent, 'function getRouteQuickAddPosition(nodes: RouteFlowNode[])', 'route designer should calculate a quick-add position below the selected operation');
mustInclude(pageContent, 'const handleRouteOperationDoubleClick = (operation: OperationRecord) => {', 'route designer should add operations from a double-click in the operation library');
mustInclude(pageContent, 'onDoubleClick={() => handleRouteOperationDoubleClick(operation)}', 'operation library items should expose the quick-add interaction');
...
mustInclude(pageContent, '<AppDialog deletionTarget={deleteTarget && { type:', 'material delete dialog should follow the system user delete dialog size and close behavior');
...
mustInclude(pageContent, `<Table stickyHeader size="small" aria-label="物料版本列表" sx={{ tableLayout: 'fixed', width: tableWidth, minWidth: tableWidth }}>`, 'material version child table should use the shared RDO table width');
...
mustInclude(pageContent, '<TableRow data-record-id={row.id} key={row.id} hover onClick={() => expandMaterialGroup(row.groupKey)}', 'material parent row click should expand or collapse child rows');
