...
mustInclude(formColumns, "{ id: 'versionCount', label: '版本数量'", 'form-template parent table should expose the aggregated version count');
...
mustInclude(pageContent, "if (columnId === 'versionCount') return String(getTemplateVersionCount(row));", 'form-template parent table should render version count');
mustInclude(pageContent, 'const shouldSubmitInitialVersionFields = !editingRow && !isVersionDialog;', 'initial version fields should be submitted only while creating a template');
mustInclude(pageContent, 'const shouldRenderVersionSection = !editingRow;', 'parent edit should hide version information section');
mustMatch(pageContent, /versionDescription: shouldSubmitVersionFields \? form\.versionDescription\.trim\(\) \|\| null : undefined,[\s\S]*version: shouldSubmitVersionFields \? form\.version\.trim\(\) : undefined,[\s\S]*effectiveFrom: shouldSubmitVersionFields \? effectiveFrom \|\| null : undefined,[\s\S]*effectiveTo: shouldSubmitVersionFields \? effectiveTo \|\| null : undefined,/, 'parent edit payload should omit hidden version date fields while preserving initial-version behavior');
...
mustNotInclude(pageContent, "isCurrent: '当前生效版本'", 'DHR version audit fields must not expose a current-version concept');
