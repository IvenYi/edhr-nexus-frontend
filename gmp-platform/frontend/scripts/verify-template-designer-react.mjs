...
if (!documentTypes.includes("export type TemplateDesignerCanvasRailKey = 'thumbnails' | 'fields' | 'grid' | 'layout';")) failures.push('document.ts: canvas side rail keys must be typed');
...
if (!storeFile.includes("activeCanvasRail: selectedRange ? 'fields' : 'thumbnails'")) failures.push('useTemplateDesignerStore.ts: cell selection should switch the side rail to field management and clearing selection should switch to thumbnails');
...
if (!canvasTab.includes("activeRail === 'fields' ? <DesignerSidebar />")) failures.push('CanvasTab.tsx: field side panel must render fields from model design');
...
if (!inspector.includes('字段信息')) failures.push('DesignerInspector.tsx: missing read-only field information section');
if (!inspector.includes('绑定关系')) failures.push('DesignerInspector.tsx: missing binding relationship section');
...
if (!inspector.includes('FieldTypeIcon')) failures.push('DesignerInspector.tsx: field information and binding options must show field type icons');
if (!inspector.includes('切换绑定字段')) failures.push('DesignerInspector.tsx: inspector must explicitly support switching bound fields');
