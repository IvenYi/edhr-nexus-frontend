...
if (!storeFile.includes("activeCanvasRail: selectedFieldNodeId ? 'config' : selectedRange ? 'fields' : 'thumbnails'")) failures.push('useTemplateDesignerStore.ts: empty cell selection should switch to field management and clearing selection should switch to thumbnails');
...
if (!storeFile.includes('findFirstCellFieldNodeIdInRange(currentPage.nodes, normalizedSelection)')) failures.push('useTemplateDesignerStore.ts: selecting a cell range must find any field component bound to that cell');
if (!storeFile.includes("activeCanvasRail: selectedFieldNodeId ? 'config' : selectedRange ? 'fields' : 'thumbnails'")) failures.push('useTemplateDesignerStore.ts: selecting a field-bearing cell edge must switch to field configuration');
