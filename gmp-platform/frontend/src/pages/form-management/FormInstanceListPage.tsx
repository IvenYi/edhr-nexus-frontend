...
type ColumnId = 'instanceNo' | 'template' | 'templateCode' | 'sourceType' | 'source' | 'workOrder' | 'operation' | 'status' | 'createdAt' | 'updatedAt';
...
const columns: ListColumn[] = [
  { id: 'instanceNo', label: '表单实例号', width: 180, minWidth: 150 },
  { id: 'template', label: '表单模板', width: 220, minWidth: 180 },
  { id: 'templateCode', label: '表单编码', width: 160, minWidth: 140 },
  ...
  { id: 'sourceType', label: '来源', width: 120, minWidth: 108 },
...
function sourceTypeLabel(type?: string | null) { return type === 'PRODUCTION_EXECUTION' ? '生产执行' : type || '-'; }
function normalizeColumnOrder(candidate: readonly ColumnId[]) {
  const known = new Set(columns.map((column) => column.id));
  const seen = new Set<ColumnId>();
  const ordered = [...candidate, ...columns.map((column) => column.id)].filter((id): id is ColumnId => {
    if (!known.has(id) || seen.has(id)) return false;
    seen.add(id);
    return true;
  });
  return [
    ...ordered.filter((id) => !FROZEN_COLUMN_IDS.includes(id)),
    ...FROZEN_COLUMN_IDS.filter((id) => ordered.includes(id)),
  ];
}
function frozenColumnSx(id: ColumnId, layer: 'head' | 'body', statusWidth: number, width: number) {
  const right = id === 'status' ? 0 : id === 'sourceType' ? statusWidth : null;
...
function readSettings(storageKey: string): ColumnSettings {
  ...
  try {
    const raw = JSON.parse(localStorage.getItem(storageKey) || 'null') as (Partial<ColumnSettings> & { layoutVersion?: number }) | null;
    const known = new Set(order);
    const versionMatches = raw?.layoutVersion === COLUMN_LAYOUT_VERSION;
    const nextOrder = versionMatches ? (raw?.order || []) : order;
    const normalizedOrder = normalizeColumnOrder(nextOrder.filter((id): id is ColumnId => known.has(id)));
    ...
    const widths = versionMatches
      ? Object.fromEntries(Object.entries(raw?.widths || {}).filter(([id, value]) => known.has(id as ColumnId) && typeof value === 'number' && Number.isFinite(value) && value > 0)) as Partial<Record<ColumnId, number>>
      : {};
...
function FormInstanceDetailDrawer({ recordId, tab, onTabChange, onClose }: { recordId: string | null; tab: number; onTabChange: (value: number) => void; onClose: () => void }) {
  ...
  return <Drawer anchor="right" open={Boolean(recordId)} onClose={onClose} sx={drawerRootSx} slotProps={{ backdrop: { sx: drawerBackdropSx } }} PaperProps={{ sx: drawerPaperSx }}>
      ...
      {detailQuery.isFetching ? <Box sx={{ py: 8, display: 'grid', placeItems: 'center' }}><CircularProgress size={24} /></Box> : detailQuery.isError ? <Typography sx={{ py: 8, textAlign: 'center', color: '#c62828' }}>表单记录加载失败</Typography> : detail ? <>
        ...
        {tab === 0 ? <Stack spacing={2} sx={{ mt: 2 }}>
          <DetailSection title="表单信息"><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.75 }}>
            <DetailField label="表单实例号" value={detail.instanceNo} /><DetailField label="表单模板" value={detail.templateName} />
            <DetailField label="表单编码" value={detail.templateCode} /><DetailField label="模板版本" value={detail.templateVersion} />
            <DetailField label="状态" value={statusBadge(detail.recordStatus)} /><DetailField label="历史记录" value={detail.legacy ? '是' : '否'} />
...
export default function FormInstanceListPage() {
  ...
  useEffect(() => {
    setSettings((current) => {
      const order = normalizeColumnOrder(current.order);
      const hidden = current.hidden.filter((id) => order.includes(id));
      if (order.every((id, index) => id === current.order[index]) && hidden.length === current.hidden.length) return current;
      return { ...current, order, hidden };
    });
  }, []);
  useEffect(() => { try { localStorage.setItem(storageKey, JSON.stringify({ ...settings, layoutVersion: COLUMN_LAYOUT_VERSION })); } catch { /* preferences are optional */ } }, [settings, storageKey]);
  ...
  const visibleTableWidth = visibleColumns.reduce((total, column) => total + columnWidth(column), 0);
  const visibleStatusColumn = visibleColumns.find((column) => column.id === 'status');
  const frozenStatusWidth = visibleStatusColumn ? columnWidth(visibleStatusColumn) : 0;
  ...
  const moveColumn = (from: ColumnId, to: ColumnId) => setSettings((current) => { const order = [...current.order]; const fromIndex = order.indexOf(from); const toIndex = order.indexOf(to); if (fromIndex < 0 || toIndex < 0 || fromIndex === toIndex) return current; order.splice(fromIndex, 1); order.splice(toIndex, 0, from); return { ...current, order: normalizeColumnOrder(order) }; });
  ...
  const renderCell = (row: GlobalFormInstanceSummary, column: ListColumn) => {
    ...
    switch (column.id) {
      ...
      case 'source': return <TableCell key={column.id} sx={cellSx} title={row.source.productionObjectNo || ''}>{row.source.productionObjectNo || '-'} <Typography component="span" variant="caption" sx={{ color: '#909399' }}>· {typeLabel(row.source.productionObjectType)}</Typography></TableCell>;
      case 'workOrder': return <TableCell key={column.id} sx={cellSx}>{row.source.workOrderNo || '-'}</TableCell>;
      case 'operation': return <TableCell key={column.id} sx={cellSx}>{row.source.operationName || '-'}</TableCell>;
      case 'status': return <TableCell key={column.id} sx={cellSx}>{statusBadge(row.recordStatus)}</TableCell>;
      case 'createdAt': return <TableCell key={column.id} sx={cellSx}>{formatDateTime(row.createdAt)}</TableCell>;
    ...
    <Box sx={formListQueryPanelSx}>
      <Box sx={formListQueryGridSx}>
        ...
        <Collapse in={advancedFiltersOpen} timeout={180} unmountOnExit sx={{ gridColumn: '1 / -1' }}>
          ...
          <TextField select size="small" label="状态" value={draft.recordStatus} onChange={(event) => setDraft({ ...draft, recordStatus: event.target.value })} sx={fieldSx}><MenuItem value="">全部</MenuItem><MenuItem value="ACTIVE">进行中</MenuItem><MenuItem value="COMPLETED">已完成</MenuItem></TextField>
