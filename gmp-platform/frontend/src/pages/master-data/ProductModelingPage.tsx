...
const tableHeaderCellSx = { bgcolor: '#f5f7fa', color: '#606266', fontWeight: 600, whiteSpace: 'nowrap', height: 48, py: 0.75 };
    ...
    <Box sx={{ flex: '0 0 auto', border: '1px solid #e4e7ed', borderRadius: 1, bgcolor: '#fff', p: 2 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5} alignItems={{ md: 'flex-end' }}>
        <Box sx={{ width: { xs: '100%', md: 340 } }}>
          <Typography variant="caption" sx={{ display: 'block', mb: 0.5, color: '#606266' }}>产品名称/编码</Typography>
          <TextField fullWidth size="small" placeholder="请输入" value={keyword} onChange={(event) => setKeyword(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') submitSearch(); }} InputProps={{ startAdornment: <InputAdornment position="start"><Search fontSize="small" /></InputAdornment> }} />
        </Box>
        <Stack direction="row" spacing={1} sx={{ ml: { md: 'auto' } }}>
          <Button variant="outlined" startIcon={<RestartAlt />} onClick={resetSearch}>重置</Button>
          <Button variant="contained" startIcon={<Search />} onClick={submitSearch}>查询</Button>
        </Stack>
      </Stack>
    </Box>
