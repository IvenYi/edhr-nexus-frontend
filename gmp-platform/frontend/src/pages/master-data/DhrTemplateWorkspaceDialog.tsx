...
} from "./template-designer-react/types";
...
function cloneComposition(
  record: DhrTemplateCompositionRecord,
): DhrTemplateCompositionRecord {
  return {
    ...record,
...
function buildDirectoryTree(
  ...
  directories.forEach((directory) =>
    nodes.set(directory.id, { ...directory, children: [], items: [] }),
  );
    ...
    entries.forEach((entry) => {
      entry.items.sort(
        (left, right) =>
          ...
          formDisplayName(left).localeCompare(formDisplayName(right)),
      );
  ...
  return nodes.flatMap((node) => {
    ...
    const items = directoryMatches
      ...
      : node.items.filter((item) =>
          [formDisplayName(item), item.formCode, item.formVersion].some(
            (entry) => entry?.toLocaleLowerCase().includes(keyword),
          ),
        );
    if (!directoryMatches && children.length === 0 && items.length === 0)
      return [];
    return [
      {
        ...node,
...
function DhrContentTree({
  ...
  useEffect(() => {
    const collectExpandableIds = (entries: DirectoryNode[]): string[] =>
  ...
  const renderForm = (
    item: DhrEvidenceItemRecord,
    depth: number,
  ): ReactElement => {
    const selected =
      selectedNode?.kind === "form" && selectedNode.id === item.id;
    return (
      <Box
        key={item.id}
        ...
        sx={{
          ...
          color: selected ? "#1890ff" : "#303133",
          "&:hover": { bgcolor: selected ? "#e8f4ff" : "#f5f7fa" },
          "&:hover .dhr-form-actions": { opacity: 1 },
        }}
      >
        <Box />
        <Stack
        ...
        >
          <ArticleOutlined
            sx={{ fontSize: 16, color: selected ? "#1890ff" : "#6c7a89" }}
          />
          <Typography
            sx={{
              minWidth: 0,
          ...
          >
            {formDisplayName(item)}
          </Typography>
          <Typography
            component="span"
            sx={{
              flexShrink: 0,
              color: selected ? "#5ca8de" : "#909399",
              fontSize: 12,
            }}
          >
            {item.formVersion}
        ...
        {editable ? (
          <Stack
            className="dhr-form-actions"
            direction="row"
            spacing={0}
            sx={{ opacity: 0, transition: "opacity .12s" }}
          >
            <Tooltip title="编辑表单" arrow>
              <IconButton
                size="small"
                aria-label="编辑表单"
                onClick={stopAnd(() => onEditForm(item))}
                sx={{ width: 25, height: 25 }}
              >
                <EditOutlined sx={{ fontSize: 16 }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="移除引用" arrow>
              <IconButton
                size="small"
                color="error"
                aria-label="移除引用表单"
                onClick={stopAnd(() => onDeleteForm(item))}
                sx={{ width: 25, height: 25 }}
              >
  ...
  const renderDirectory = (
    ...
    const selected =
      selectedNode?.kind === "directory" && selectedNode.id === node.id;
    return (
      <Box key={node.id}>
        <Box
          role="treeitem"
          ...
          sx={{
            ...
            color: selected ? "#1890ff" : "#303133",
            "&:hover": { bgcolor: selected ? "#e8f4ff" : "#f5f7fa" },
            "&:hover .dhr-directory-actions": { opacity: 1 },
          }}
        >
          {hasChildren ? (
            <IconButton
          ...
          >
            {hasChildren && expanded ? (
              <FolderOpenOutlined sx={{ fontSize: 17, color: "#d9a441" }} />
            ) : (
              <FolderOutlined sx={{ fontSize: 17, color: "#d9a441" }} />
            )}
            <Typography
              sx={{
                minWidth: 0,
          ...
          {editable ? (
            <Stack
              className="dhr-directory-actions"
              direction="row"
              spacing={0}
              sx={{ opacity: 0, transition: "opacity .12s" }}
            >
              <Tooltip title="引用表单" arrow>
                <IconButton
                  size="small"
                  aria-label="引用表单"
                  onClick={stopAnd(() => onAddForm(node))}
                  sx={{ width: 25, height: 25 }}
                >
                  <NoteAddOutlined sx={{ fontSize: 16 }} />
                </IconButton>
              </Tooltip>
              <Tooltip title="新增子目录" arrow>
                <IconButton
                  size="small"
                  aria-label="新增子目录"
                  onClick={stopAnd(() => onAddChild(node))}
                  sx={{ width: 25, height: 25 }}
                >
                  <PostAddRounded sx={{ fontSize: 16 }} />
                </IconButton>
              </Tooltip>
              <Tooltip title="重命名" arrow>
                <IconButton
                  size="small"
                  aria-label="重命名目录"
                  onClick={stopAnd(() => onRename(node))}
                  sx={{ width: 25, height: 25 }}
                >
                  <DriveFileRenameOutlineRounded sx={{ fontSize: 16 }} />
                </IconButton>
              </Tooltip>
              <Tooltip title="删除" arrow>
                <IconButton
                  ...
                  color="error"
                  aria-label="删除目录"
                  onClick={stopAnd(() => onDeleteDirectory(node))}
                  sx={{ width: 25, height: 25 }}
                >
                  <DeleteOutlineRounded sx={{ fontSize: 16 }} />
                </IconButton>
        ...
        {hasChildren && expanded ? (
          <>
            {node.children.map((child) => renderDirectory(child, depth + 1))}
            {node.items.map((item) => renderForm(item, depth + 1))}
          </>
        ) : null}
      </Box>
    );
  ...
  return (
    <Box role="tree" sx={{ py: 0.5 }}>
      {nodes.map((node) => renderDirectory(node, 0))}
    </Box>
  );
}
...
function flattenCanvasNodes(nodes: CanvasNode[]): CanvasNode[] {
  return nodes.flatMap((node) => [
    node,
    ...(node.children ? flattenCanvasNodes(node.children) : []),
  ]);
}
...
export type PreviewFieldAction = {
  ...
  title: string;
  icon: ReactElement;
  onClick: (fieldId: string, anchor: HTMLElement) => void;
};

export type PreviewFieldInteraction = {
...
function PreviewFieldActionBar({
  fieldId,
  actions,
}: {
  fieldId: string;
  actions: PreviewFieldAction[];
}) {
  if (!actions.length) return null;
  return (
    <Box
      className="preview-field-actions"
      sx={{
        position: "absolute",
        top: 2,
        right: 2,
      ...
      {actions.map((action) => (
        <Tooltip key={action.key} title={action.title} arrow>
          <IconButton
            ...
            onClick={(event) => {
              event.stopPropagation();
              action.onClick(fieldId, event.currentTarget);
            }}
...
function PreviewField({
  ...
  const label = String(
    node.bindings?.displayLabel || field?.name || node.props.label || "字段",
  );
  const placeholder = String(
    node.bindings?.placeholder ||
      node.props.placeholder ||
      (field?.type === "datetime"
        ? "请选择日期"
        : field?.type === "singleSelect" || field?.type === "reference"
          ? "请选择"
          : "请输入"),
  );
  ...
  const readOnly =
    ...
    Boolean(node.bindings?.readonly);
  const highlighted =
    Boolean(fieldId) && interaction?.highlightFieldId === fieldId;
  const actions =
    fieldId && field
      ? (interaction?.actionsForField?.({
          id: field.id,
          name: field.name,
          type: field.type,
        }) ?? [])
  ...
  return (
    <Box
      onMouseEnter={() => fieldId && interaction?.onFieldHover?.(fieldId)}
      onMouseLeave={() => interaction?.onFieldHover?.(null)}
      sx={{
        position: "relative",
        height: "100%",
        minWidth: 0,
        ...
        alignItems: "center",
        px: 0.75,
        color: readOnly ? "#7b8794" : "#617083",
        fontSize: 12,
        border: `1px solid ${highlighted ? "#1677c8" : readOnly ? "#d9dee7" : "#c7d8e8"}`,
        borderRadius: "3px",
        bgcolor: highlighted ? "#eaf4ff" : readOnly ? "#f3f5f7" : "#fff",
        boxShadow: highlighted ? "0 0 0 2px rgba(22,119,200,.16)" : "none",
        overflow: "hidden",
        whiteSpace: "nowrap",
        textOverflow: "ellipsis",
...
function SheetPreview({
  ...
  const rows = page.sheet.rowHeights
    .slice(0, page.sheet.rowCount)
    .map((height) => Math.max(24, Math.min(180, height)));
  ...
  page.mergedCells.forEach((range) => {
    ...
    for (let row = range.t; row <= range.b; row += 1)
      for (let col = range.l; col <= range.r; col += 1)
        if (row !== range.t || col !== range.l)
          mergedSkips.add(`${row}:${col}`);
  });
  const columnOffsets = columns.reduce<number[]>(
    (offsets, width) => [...offsets, offsets[offsets.length - 1] + width],
    [0],
  );
  const rowOffsets = rows.reduce<number[]>(
    (offsets, height) => [...offsets, offsets[offsets.length - 1] + height],
    [0],
  );
  ...
  return (
    <Box
      sx={{ flex: 1, minHeight: 0, overflow: "auto", bgcolor: "#eef3f8", p: 3 }}
    >
      <Box
        sx={{
          display: "flex",
      ...
      >
        ...
        >
          ...
          >
            {Array.from(
              { length: page.sheet.rowCount },
              (_, rowIndex) => rowIndex + 1,
            ).flatMap((row) =>
              Array.from(
                { length: page.sheet.columnCount },
          ...
          {nodeLayers.map(({ node, left, top, width, height }) => (
            <Box
              key={node.id}
              sx={{
                position: "absolute",
      ...
      >
        {groups.map((group) => {
          const groupFields = fields.filter(
            (field) => (field.groupId ?? "default-group") === group.id,
          );
                ...
                {groupFields.map((field) => {
                  ...
                  return (
                    <Box key={field.id}>
                      ...
                      <Box
                        onMouseEnter={() =>
                          interaction?.onFieldHover?.(field.id)
                        }
                        onMouseLeave={() => interaction?.onFieldHover?.(null)}
                        sx={{
                          position: "relative",
  ...
  return (
    ...
    >
      {document.canvas.pages.length > 1 ? (
        <Tabs
          value={page?.id ?? false}
          onChange={(_, value) => setPageId(value)}
          sx={{
            minHeight: 40,
            px: 1.5,
            borderBottom: "1px solid #e4e7ed",
        ...
        >
          {document.canvas.pages.map((entry) => (
            <Tab key={entry.id} value={entry.id} label={entry.name} />
          ))}
        </Tabs>
      ) : null}
...
export default function DhrTemplateWorkspaceDialog({
  ...
  const [directoryDialog, setDirectoryDialog] =
    useState<DirectoryDialogState | null>(null);
  ...
  >(() => new Map());
  const [expandedFormTemplateIds, setExpandedFormTemplateIds] = useState<
    Set<string>
  >(() => new Set());
  ...
  const [editEvidenceTarget, setEditEvidenceTarget] =
    useState<DhrEvidenceItemRecord | null>(null);
  const [evidenceDisplayName, setEvidenceDisplayName] = useState("");
  const [baseComposition, setBaseComposition] =
    useState<DhrTemplateCompositionRecord | null>(null);
  const [compositionDraft, setCompositionDraft] =
    useState<DhrTemplateCompositionRecord | null>(null);
  const [appliedCompositionRevision, setAppliedCompositionRevision] =
    useState("");
  ...
  const formOptionsQuery = useQuery({
    ...
    enabled: open && Boolean(templateId),
    queryFn: async () =>
      (await getDhrFormTemplateOptions(templateId!)).data.data,
  });
  const compositionQuery = useQuery({
    ...
    queryFn: async () =>
      (await getDhrTemplateComposition(templateId!, selectedVersionId!)).data
        .data,
  });
  ...
  const selectedVersion =
    ...
    compositionQuery.data?.version ??
    null;
  const compositionRevision = compositionQuery.data
    ? `${compositionQuery.data.version.id}:${compositionQuery.dataUpdatedAt}`
    : "";
  ...
  const directoryTree = useMemo(
    () => buildDirectoryTree(directories, items),
    [directories, items],
  );
  const visibleDirectoryTree = useMemo(
    () => filterDirectoryTree(directoryTree, directorySearch),
    [directorySearch, directoryTree],
  );
  const editable = Boolean(selectedVersion);
  const selectedTreeNode = selectedEvidence
    ? { kind: "form" as const, id: selectedEvidence.id }
    : selectedDirectory
      ? { kind: "directory" as const, id: selectedDirectory.id }
      : null;
  const formPreviewQuery = useQuery({
    queryKey: [
      "dhr-referenced-form-version",
      selectedEvidence?.formTemplateId,
      selectedEvidence?.formTemplateVersionId,
    ],
    enabled:
      open &&
      Boolean(selectedEvidence?.formTemplateId) &&
      Boolean(selectedEvidence?.formTemplateVersionId),
    queryFn: async () =>
      (
  ...
  const formCategories = useMemo(
    () =>
      Array.from(
        new Set(
          (formOptionsQuery.data ?? []).map(
            (option) => option.categoryName?.trim() || "未分类",
          ),
        ),
  ...
  const visibleFormOptions = useMemo(() => {
    ...
    return (formOptionsQuery.data ?? []).filter((option) => {
      ...
      return (
        (formCategory === "ALL" || categoryName === formCategory) &&
  ...
  const selectedFormReferences = useMemo(
    () => Array.from(selectedFormOptions.values()),
    [selectedFormOptions],
  );
  const referencedFormIds = useMemo(
    () =>
      new Set(
        items
          .filter((item) => item.directoryId === selectedDirectoryId)
  ...
  const invalidateWorkspace = async () => {
    ...
    await queryClient.invalidateQueries({
      queryKey: ["dhr-template-composition", templateId],
    });
    await queryClient.invalidateQueries({
  ...
  useEffect(() => {
    if (
      selectedEvidenceId &&
      !items.some((item) => item.id === selectedEvidenceId)
    )
      setSelectedEvidenceId(null);
  ...
  useEffect(() => {
    ...
    if (
      hasChanges ||
      (appliedCompositionRevision === compositionRevision && compositionDraft)
    )
      return;
  ...
  useEffect(() => {
    ...
    setExpandedFormTemplateIds(
      new Set((formOptionsQuery.data ?? []).map((option) => option.templateId)),
    );
  ...
  const selectDirectory = (directory: DhrDirectoryRecord) => {
    setSelectedDirectoryId(directory.id);
    setSelectedEvidenceId(null);
  };
  const selectForm = (item: DhrEvidenceItemRecord) => {
    setSelectedDirectoryId(item.directoryId);
  ...
  const asSelectedFormReference = (
    templateOption: DhrFormTemplateOption,
    version: DhrFormTemplateOption["versions"][number],
  ...
  const toggleFormTemplateExpanded = (
    templateOption: DhrFormTemplateOption,
  ) => {
    setExpandedFormTemplateIds((current) => {
      const next = new Set(current);
      if (next.has(templateOption.templateId))
        next.delete(templateOption.templateId);
      else next.add(templateOption.templateId);
      return next;
    });
  };
  const toggleFormVersionSelection = (
    templateOption: DhrFormTemplateOption,
    version: DhrFormTemplateOption["versions"][number],
  ) => {
    if (
      !version.referenceable ||
      referencedFormIds.has(templateOption.templateId)
    )
      return;
    setSelectedFormOptions((current) => {
      ...
      next.set(
        version.versionId,
        asSelectedFormReference(templateOption, version),
      );
      return next;
    });
  };
  const toggleTemplateSelection = (templateOption: DhrFormTemplateOption) => {
    ...
    const referenceableVersions = templateOption.versions.filter(
      (version) => version.referenceable,
    );
    if (referenceableVersions.length !== 1) {
      setExpandedFormTemplateIds((current) =>
        new Set(current).add(templateOption.templateId),
      );
      return;
    }
    setSelectedFormOptions((current) => {
        ...
        current
          ? {
              ...
              directories: current.directories.map((directory) =>
                directory.id === directoryDialog.target?.id
  ...
  const requestDeleteDirectory = (directory: DhrDirectoryRecord) => {
    if (
      ...
      items.some((item) => item.directoryId === directory.id)
    ) {
      setSnackbar({
        open: true,
        message: "请先移除目录下的子目录和表单",
        severity: "error",
      });
      return;
    }
  ...
  const stageDeleteDirectory = () => {
    ...
    setCompositionDraft((current) =>
      current
        ? {
            ...current,
            directories: current.directories.filter(
              (directory) => directory.id !== deleteDirectoryTarget.id,
            ),
          }
        : current,
    );
  ...
  const stageEvidence = () => {
    ...
    const existingTemplateIds = new Set(
      items
        .filter((candidate) => candidate.directoryId === selectedDirectoryId)
        .map((candidate) => candidate.formTemplateId),
    );
    const stagedTemplateIds = new Set<string>();
    for (const reference of selectedFormReferences) {
      if (
        existingTemplateIds.has(reference.templateId) ||
        stagedTemplateIds.has(reference.templateId)
      ) {
        setSnackbar({
          open: true,
          message: "同一目录不能重复引用同一表单",
          severity: "error",
        });
        return;
      }
    ...
    const stagedItems: DhrEvidenceItemRecord[] = selectedFormReferences.map(
      (option, index) => ({
    ...
    setCompositionDraft((current) =>
      current
        ? { ...current, items: [...current.items, ...stagedItems] }
        : current,
    );
  ...
  const stageDeleteEvidence = () => {
    ...
    setCompositionDraft((current) =>
      current
        ? {
            ...current,
            items: current.items.filter((item) => item.id !== deleteEvidenceId),
          }
        : current,
    );
  ...
  const stageEvidenceDisplayName = () => {
    ...
    const displayName =
      evidenceDisplayName.trim() === editEvidenceTarget.formName
        ? null
        : evidenceDisplayName.trim();
    setCompositionDraft((current) =>
      current
        ? {
            ...current,
            items: current.items.map((item) =>
              item.id === editEvidenceTarget.id
                ? { ...item, displayName }
                : item,
            ),
          }
  ...
  const saveCompositionMutation = useMutation({
    mutationFn: async () => {
      if (!templateId || !selectedVersionId || !compositionDraft)
        throw new Error("批记录设计上下文缺失");
      const draftDirectoryMap = new Map(
        compositionDraft.directories.map((directory) => [
          directory.id,
          directory,
        ]),
      );
      ...
      for (const item of compositionDraft.items) {
        if (!draftDirectoryMap.has(item.directoryId))
          throw new Error("引用表单缺少所属目录");
        const duplicateKey = `${item.directoryId}:${item.formTemplateId}`;
        if (duplicateReferences.has(duplicateKey))
          throw new Error("同一目录不能重复引用同一表单");
  ...
  return (
    <AppDialog
      ...
      fullScreen
      PaperProps={{ sx: { borderRadius: 0, bgcolor: "#f6f8f9" } }}
    >
      <Box
        sx={{
          height: "100vh",
      ...
      >
        ...
        >
          <Typography
            sx={{ color: "#303133", fontSize: 16, fontWeight: 600, mr: "auto" }}
          >
            批记录模板设计
          </Typography>
        ...
        <Box sx={{ bgcolor: "#fff", borderBottom: "1px solid #e4e7ed" }}>
          <Stack
            direction="row"
            alignItems="center"
            sx={{ minHeight: 48, px: 2, borderBottom: "1px solid #eef0f3" }}
          >
            <Button
              color="inherit"
              size="small"
              ...
              aria-label={
                basicInfoExpanded ? "收起基本信息详情" : "展开基本信息详情"
              }
              onClick={() => setBasicInfoExpanded((current) => !current)}
              startIcon={
                <ExpandMoreRounded
                  sx={{
                    transform: basicInfoExpanded
                      ? "rotate(0deg)"
                      : "rotate(-90deg)",
            ...
            >
              <Typography noWrap sx={{ color: "#606266", fontSize: 13 }}>
                设计版本 {selectedVersion?.version ?? "-"}
              </Typography>
              <StatusBadge
                {...getRdoVersionStatusMeta(selectedVersion?.status)}
              />
            </Stack>
          </Stack>
          <Box
          ...
          >
            <Box>
              ...
              <Typography
                noWrap
                sx={{ color: "#303133", fontSize: 14, fontWeight: 600 }}
              >
                {template?.name ?? "-"}
              </Typography>
            </Box>
            <Box>
              ...
              <Typography noWrap sx={{ color: "#303133", fontSize: 14 }}>
          ...
          <Collapse in={basicInfoExpanded} timeout="auto">
            ...
            >
              ...
              <Box>
                ...
                <Typography noWrap sx={{ color: "#303133", fontSize: 14 }}>
                  {selectedVersion?.effectiveFrom || "未设置"}
                </Typography>
              </Box>
              <Box>
                <Typography variant="caption" sx={{ color: "#909399" }}>
                ...
                <Typography noWrap sx={{ color: "#303133", fontSize: 14 }}>
                  {selectedVersion?.effectiveTo || "未设置"}
                </Typography>
              </Box>
              <Box>
                <Typography variant="caption" sx={{ color: "#909399" }}>
                ...
                <Typography noWrap sx={{ color: "#303133", fontSize: 14 }}>
                  {selectedVersion?.offlineVersion || "未填写"}
                </Typography>
              </Box>
              <Box>
                <Typography variant="caption" sx={{ color: "#909399" }}>
                ...
                <Typography noWrap sx={{ color: "#303133", fontSize: 14 }}>
                  {selectedVersion?.description || "未填写"}
                </Typography>
              </Box>
              <Box>
                <Typography variant="caption" sx={{ color: "#909399" }}>
                ...
                <Typography sx={{ color: "#303133", fontSize: 14 }}>
                  {directories.length}
                </Typography>
              </Box>
              <Box>
        ...
        >
          ...
          >
            <Stack
              spacing={1}
              sx={{ px: 1.5, py: 1, borderBottom: "1px solid #e4e7ed" }}
            >
              <Stack
                direction="row"
                alignItems="center"
              ...
              >
                <Typography sx={{ fontWeight: 600, color: "#303133" }}>
                  DHR 目录
                </Typography>
                <Tooltip title="新增根目录" arrow>
                  <span>
                    <IconButton
                      size="small"
                      aria-label="新增根目录"
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <SearchRounded sx={{ color: "#909399", fontSize: 18 }} />
                    </InputAdornment>
                  ),
                }}
            ...
            <Box sx={{ flex: 1, minHeight: 0, overflow: "auto" }}>
              {compositionQuery.isLoading || !compositionDraft ? (
                <Box
                  sx={{ display: "grid", placeItems: "center", height: "100%" }}
                >
                  <CircularProgress size={24} />
                </Box>
              ) : visibleDirectoryTree.length ? (
                <DhrContentTree
                  nodes={visibleDirectoryTree}
                  selectedNode={selectedTreeNode}
                  editable={editable && !saveCompositionMutation.isPending}
                  onSelectDirectory={selectDirectory}
                  onSelectForm={selectForm}
                  onAddForm={openAddEvidence}
                  onAddChild={(directory) => openCreateDirectory(directory.id)}
                  onRename={openEditDirectory}
                  onDeleteDirectory={requestDeleteDirectory}
                  onDeleteForm={(item) => setDeleteEvidenceId(item.id)}
                  onEditForm={openEditEvidence}
                />
              ) : (
                <Box sx={{ p: 2, color: "#909399", fontSize: 13 }}>
                  {directorySearch.trim() ? "暂无匹配的目录或表单" : "暂无目录"}
                </Box>
              )}
            </Box>
          </Box>
          ...
          >
            {selectedEvidence ? (
              <>
                ...
                >
                  <ArticleOutlined sx={{ color: "#5b7188", fontSize: 20 }} />
                  <Stack spacing={0.1} sx={{ minWidth: 0, mr: "auto" }}>
                    <Typography
                      noWrap
                      sx={{ color: "#303133", fontSize: 14, fontWeight: 600 }}
                    >
                      {formDisplayName(selectedEvidence)}
                    </Typography>
                    <Typography
                      noWrap
                      variant="caption"
                      sx={{ color: "#909399" }}
                    >
                      {selectedEvidence.formCode} ·{" "}
                      {selectedEvidence.formVersion}
                    </Typography>
                  </Stack>
                </Stack>
              ...
              >
                <Typography sx={{ fontSize: 13 }}>请选择左侧目录</Typography>
              </Box>
            )}
          </Box>
        </Box>
        ...
        >
          ...
          <Button
            ...
            disabled={
              !hasChanges || !editable || saveCompositionMutation.isPending
            }
          >
            {saveCompositionMutation.isPending ? "保存中" : "保存"}
          </Button>
        </Stack>
      </Box>

      <AppDialog
        open={Boolean(directoryDialog)}
        onClose={() => setDirectoryDialog(null)}
        fullWidth
        maxWidth="xs"
      >
        <DialogTitle>
          {directoryDialog?.mode === "edit" ? "重命名目录" : "新增目录"}
        </DialogTitle>
        <DialogContent dividers>
          <TextField
            ...
            sx={{ mt: 0.5, "& .MuiInputBase-root": { height: 40 } }}
          />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDirectoryDialog(null)}>取消</Button>
          <Button
            variant="contained"
      ...
      <AppDialog
        ...
        onClose={() => setDeleteDirectoryTarget(null)}
        fullWidth
        maxWidth="xs"
      >
        <DialogTitle>删除目录</DialogTitle>
        <DialogContent dividers>
          <Typography>确认删除“{deleteDirectoryTarget?.name}”？</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteDirectoryTarget(null)}>取消</Button>
          <Button
            color="error"
            variant="contained"
            onClick={stageDeleteDirectory}
          >
            删除
          </Button>
        </DialogActions>
      ...
      >
        ...
        >
          <Typography sx={{ color: "#303133", fontSize: 17, fontWeight: 600 }}>
            批量引用表单
          </Typography>
        </DialogTitle>
        <DialogContent
        ...
        >
          ...
          >
            ...
            >
              <Table stickyHeader size="small" sx={{ minWidth: 720 }}>
                <TableHead>
                  <TableRow sx={{ "& .MuiTableCell-root": tableHeaderSx }}>
                    <TableCell padding="checkbox" />
                    <TableCell>表单名称</TableCell>
                    <TableCell>表单编码</TableCell>
                    <TableCell>表单分类</TableCell>
                ...
                <TableBody>
                  {formOptionsQuery.isLoading ? (
                    ...
                    visibleFormOptions.flatMap((option) => {
                      const alreadyReferenced = referencedFormIds.has(
                        option.templateId,
                      );
                      const referenceableVersions = option.versions.filter(
                        (version) => version.referenceable,
                      );
                      const selectedVersionCount = referenceableVersions.filter(
                        (version) => selectedFormOptions.has(version.versionId),
                      ).length;
                      const selected = selectedVersionCount > 0;
                      const expanded = expandedFormTemplateIds.has(
                        option.templateId,
                      );
                      const categoryName =
                        option.categoryName?.trim() || "未分类";
                      const parentCheckboxDisabled =
                        alreadyReferenced || referenceableVersions.length !== 1;
                      const parentCheckboxTooltip = alreadyReferenced
                        ? "当前目录已引用该表单"
                        : referenceableVersions.length === 0
                          ? "没有可引用的生效版本"
                          : referenceableVersions.length === 1
                            ? "选择生效版本"
                            : "请展开选择具体版本";
                      const parentRow = (
                        <TableRow
                          ...
                          hover
                          selected={selected}
                          onClick={() => toggleFormTemplateExpanded(option)}
                          sx={{
                            cursor: "pointer",
                            "& .MuiTableCell-root": {
                              height: 44,
                        ...
                        >
                          ...
                          >
                            <Tooltip title={parentCheckboxTooltip} arrow>
                              <span>
                                <Checkbox
                                  size="small"
                                  checked={
                                  ...
                                  onChange={() =>
                                    toggleTemplateSelection(option)
                                  }
                                />
                              </span>
                            </Tooltip>
                          ...
                          <TableCell>
                            ...
                            >
                              <IconButton
                                ...
                                onClick={(event) => {
                                  event.stopPropagation();
                                  toggleFormTemplateExpanded(option);
                                }}
                                sx={{ width: 24, height: 24 }}
                              >
                                {expanded ? (
                          ...
                          <TableCell>{option.updatedAt || "-"}</TableCell>
                        </TableRow>
                      );
                      const versionRows = expanded
                        ? option.versions.map((version) => {
                            const unavailable =
                              alreadyReferenced || !version.referenceable;
                            const selectedVersion = selectedFormOptions.has(
                              version.versionId,
                            );
                            const unavailableReason = alreadyReferenced
                              ? "当前目录已引用该表单"
                              : "仅可引用生效中的版本";
                            return (
                              <TableRow
                                ...
                                onClick={() =>
                                  toggleFormVersionSelection(option, version)
                                }
                                sx={{
                                  cursor: unavailable
                                    ? "not-allowed"
                                    : "pointer",
                                  opacity:
                                    unavailable && !selectedVersion ? 0.62 : 1,
                                  "& .MuiTableCell-root": {
                                    height: 40,
                                    bgcolor: selectedVersion
                                      ? "#e8f4ff"
                                      : "#fbfcfe",
                                  },
                                }}
          ...
          >
            ...
            >
              <Typography
                sx={{ color: "#303133", fontSize: 15, fontWeight: 600 }}
              >
                已选表单
              </Typography>
              <Typography sx={{ color: "#909399", fontSize: 12 }}>
                已选择 {selectedFormReferences.length} 个版本
              </Typography>
            </Stack>
            <Box sx={{ flex: 1, minHeight: 0, overflow: "auto", p: 1.25 }}>
              {selectedFormReferences.length ? (
                <Stack spacing={0.75}>
                  {selectedFormReferences.map((option) => (
                    <Stack
                    ...
                    >
                      ...
                      <Tooltip title="移除" arrow>
                        <IconButton
                          size="small"
                          aria-label={`移除${option.name}${option.version}`}
                          onClick={() =>
                            setSelectedFormOptions((current) => {
                              const next = new Map(current);
                              next.delete(option.versionId);
                              return next;
                            })
                          }
                          sx={{ flexShrink: 0, width: 26, height: 26 }}
                        >
        ...
        <DialogActions
          sx={{ minHeight: 62, px: 2.5, borderTop: "1px solid #e4e7ed" }}
        >
          <Button onClick={() => setAddEvidenceOpen(false)}>取消</Button>
          <Button
            variant="contained"
            disabled={!selectedFormReferences.length}
      ...
      <AppDialog
        ...
        onClose={() => setEditEvidenceTarget(null)}
        fullWidth
        maxWidth="sm"
      >
        ...
        <DialogContent dividers>
          <Stack spacing={2} sx={{ pt: 0.5 }}>
            ...
            >
              ...
              >
                ...
                <Box sx={{ minWidth: 0 }}>
                  ...
                  >
                    {editEvidenceTarget?.formVersion ?? "-"}
                  </Typography>
                </Box>
              </Box>
            </Box>
            <TextField
              ...
              onChange={(event) => setEvidenceDisplayName(event.target.value)}
              inputProps={{ maxLength: 256 }}
              helperText={`${evidenceDisplayName.length} / 256`}
            />
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setEditEvidenceTarget(null)}>取消</Button>
          <Button
            variant="contained"
            disabled={!evidenceDisplayName.trim()}
            onClick={stageEvidenceDisplayName}
          >
            确定
          </Button>
        </DialogActions>
      ...
      <AppDialog
        ...
        maxWidth="xs"
      >
        <DialogTitle>移除引用表单</DialogTitle>
        <DialogContent dividers>
          <Typography>确认移除该引用表单？</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteEvidenceId(null)}>取消</Button>
          <Button
            color="error"
            variant="contained"
            onClick={stageDeleteEvidence}
          >
            移除
          </Button>
        </DialogActions>
      ...
      >
        ...
        <DialogContent dividers>
          <Typography>目录和表单引用的修改尚未保存，确认放弃吗？</Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDiscardConfirm(false)}>继续编辑</Button>
