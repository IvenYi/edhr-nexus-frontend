...
import FormFillSettings from "@/components/form-fill-settings/FormFillSettings";
import { type FormFieldOption, type ProcessPermissionSubject } from "@/components/form-fill-settings/FormFieldPermissions";
import { fillMode, type FormFillSettingsValue } from "@/components/form-fill-settings/types";
...
function validateFlowGraphForPublish(
  ...
  graph.nodes.forEach((node) => {
    ...
    if (node.data.kind === "FORM") {
      ...
      if (fillMode(node.data.config ?? {}) === "PROCESS" && !node.data.config?.formProcessVersionId) {
        addNodeError(node.id, "请选择已发布的表单流程");
      }
      if (!formVersionId) {
...
function nodeCaption(data: WorkNodeData) {
  ...
  if (data.kind === "FORM")
    return [config.formTemplateName || "选择表单", fillMode(config) === "DIRECT" ? "直接填报" : config.formProcessName || "待选流程"]
      .filter(Boolean)
      .join(" · ");
...
>(
  (
    ...
    const outdatedFormProcessReferences = (
      ...
      if (!localNode || !localVersionId || fillMode(localNode.data.config ?? {}) !== "PROCESS") return false;
    ...
    const selectedOutdatedFormProcessReference =
      fillMode(selectedNode?.data.config ?? {}) === "PROCESS" &&
          ...
          {selectedNode ? (
            <>
              ...
              >
                ...
                {editable && selectedKind !== "START" && selectedKind !== "END" ? (
                  <Tooltip title="删除节点">
                    <IconButton size="small" aria-label="删除节点" color="error" onClick={requestDeleteSelectedNode}>
              ...
              <Stack spacing={1.75} sx={{ p: 1.5 }}>
                ...
                {selectedKind === "FORM" ? (
                  <>
                    ...
                    <FormFillSettings key={selectedNode.id} value={selectedNode.data.config ?? {}} form={selectedFormOption} editable={editable} onChange={(config) => updateSelectedNode({ config })} />
                  </>
                ) : null}
