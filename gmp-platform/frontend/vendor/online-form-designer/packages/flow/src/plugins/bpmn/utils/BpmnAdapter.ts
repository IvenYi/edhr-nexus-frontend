...
import type { ITransformOpts, ITransformResult, GctBpmnNode, IGctBpmnNode, ICase } from '../types';
import { BpmnNodeTypeEnum, CaseValueSource } from '../enums';
...
function getValueByType(type: CaseValueSource, value: any, subIdx?: number, subValue?: any): any {
  return type === CaseValueSource.Model
    ? '${' + (subValue ? `${value}[${subIdx}].${subValue}` : value) + '}'
    : type === CaseValueSource.Node
      ? '${btn_' + value + '}'
      : value;
}

function caseJsonTransfer(json: ICase): any {
  json.elements.forEach((item: any) => {
    if (item.type === 'condition') {
      ...
      item.element = {
        type: lSubType || type,
        operator: lSubOperator || operator,
        left: getValueByType(lType, lValue, lSubIndex, lSubValue),
        right: getValueByType(rType, rValue, rSubIndex, rSubValue),
      };
    } else {
      caseJsonTransfer(item.element);
    }
  });
  return json;
}
...
const buildXmlJson = function (data, callback?: Function): any {
  ...
  const bpmnNodeArrayTmp: BpmnItem[] = [];
    ...
    nodes.forEach((node) => {
      ...
      if (
        [
          ...
          BpmnNodeTypeEnum.BpmnInclusiveS,
  ...
  for (const item of bpmnFlowSet.keys()) {
    ...
    if (targetRef.includes(':')) {
      ...
      if (fn.caseCfg.json) {
        const caseJson = JSON.parse(JSON.stringify(fn.caseCfg.json));
        let text = '';
        if (fn.caseCfg.type === 'FORMULA') {
          ...
          text =
            '<![CDATA[${choiceExecutor.executeExp(execution,' +
            `'${Base64.encode(json)}'` +
            ')}]]>';
        } else {
          text =
            '<![CDATA[${choiceExecutor.execute(execution,' +
            `'${Base64.encode(JSON.stringify(caseJsonTransfer(caseJson)))}'` +
            ')}]]>';
        }
        sf.children = [
          {
  ...
  const bpmnNodeArray = bpmnNodeArrayTmp.map((n) => {
    ...
    node.children ??= [];
    const incoming = bpmnFlowArray.filter((item) => item.attrs.targetRef === node.attrs.id);
    const outgoing = bpmnFlowArray.filter((item) => item.attrs.sourceRef === node.attrs.id);

    if (n.nodeType === BpmnNodeTypeEnum.BpmnInclusiveS) {
      node.attrs.default = outgoing[outgoing.length - 1].attrs.id;
    }
