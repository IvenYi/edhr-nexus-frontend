...
import type { ITransformResult, GctBpmnNode, IGctBpmnNode } from '../types';
...
function caseXml(formulaType, json, isBusiness, caseId) {
  if (formulaType) {
    if (isBusiness) {
      return '<![CDATA[${' + 'bizProcChoiceExecutor.executeExp(execution,' + `'${json}'` + ')}]]>';
    } else {
      return (
        '<![CDATA[${pmChoiceExecutor.executeExp(execution,' + `'${json}',` + `'${caseId}'` + ')}]]>'
      );
    }
  } else {
    if (isBusiness) {
      ...
      return (
        '<![CDATA[${pmChoiceExecutor.execute(execution,' + `'${json}',` + `'${caseId}'` + ')}]]>'
      );
    }
  }
}
 ...
 * @returns
 */
const buildXmlJson = function (data, isBusiness = false, callback?: Function): any {
    ...
    if (targetRef.includes(':')) {
      ...
      if (fn.caseCfg.type) {
        if (fn.caseCfg.type === 'FORMULA') {
          ...
          text = caseXml(true, Base64.encode(json), isBusiness, caseId);
          // text =
          //   '<![CDATA[${' +
          //   `${isBusiness ? 'bizProcChoiceExecutor' : 'pmChoiceExecutor'}` +
          //   '.executeExp(execution,' +
          //   `'${Base64.encode(json)}',` +
          //   `'${caseId}'` +
          //   ')}]]>';
          ...
          text = caseXml(false, Base64.encode(JSON.stringify(json)), isBusiness, caseId);
          // text =
          //   '<![CDATA[${' +
          //   `${isBusiness ? 'bizProcChoiceExecutor' : 'pmChoiceExecutor'}` +
          //   '.execute(execution,' +
...
export class BpmnAdapter {
  static transfer(data, options?, isBusiness = false): ITransformResult {
    const nodes: any[] = [];
    const xmlJson = buildXmlJson(data, isBusiness, (node) => {
      nodes.push(node);
    });
    /** 设置流程 id */
