  ...
  >
    ...
    <template #footer>
      <div class="flex">
        ...
        <van-button class="flex-1" type="primary" @click="onOk">{{
          labelInfo.submitBtnTitle
        }}</van-button>
      </div>
    </template>
  </BasicPopup>
...
<script setup lang="ts" name="form-abandon-v2-popup">
  ...
  import { openSelectProcessPopup } from '../process';
  ...
  const labelInfo = computed(() => {
    if (props.isFormChange) {
      return {
        popupTitle: t('sys.onlineForm.formChange'),
  ...
  const onOk = async () => {
    try {
      ...
      const res = await openSelectProcessPopup({
        title: '流程选择',
        categoryId: '__change_process__',
      });

      if (res) {
        console.log('流程选择', res);
        const data = { ...formData, approveTmplId: res.id };
        doClose(data);
      }
    } catch (err) {
      console.error(err);
