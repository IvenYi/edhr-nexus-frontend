...
export function getEdhrConfig(mode: string) {
  ...
  return {
    ...
    plugins: [
      ...
      createSvgIconsPlugin({
        iconDirs: [resolve('./', 'src/assets/svg-edhr'), resolve('../../', 'src/assets/icons')],
      }),
      createHtmlPlugin({
        minify: true,
        pages: [
          {
            entry: '/src/views/edhr/main.ts',
            filename: 'index.html',
            template: 'index.html',
            injectOptions: {
              data: {
      ...
      compresssionBuild({
        sourceName: `../../dist/build/src/projects/edhr-render`,
        type: 'zip',
        targetName: `../../dist/build/src/projects/edhr`,
        ignoreBase: true,
      }),
    ],
    resolve: {
    ...
    build: {
      outDir: '../../dist/build/src/projects/edhr-render/dist',
    },
    server: {
      port: 7777,
      host: true,
