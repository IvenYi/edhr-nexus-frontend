...
export interface AppOrgUserPageRequest {
  ...
  keyword?: string; // 关键字
  keywordFields?: any[]; // 关键字筛选字段
  orgId?: string; // 部门id
  orgIds?: any[]; // 部门id，多个的情况
  pageNo?: number; // 页码
...
export interface SearchAllOrgOrUserResponse {
  orgIdList?: any[]; // 部门id集合
  userIdList?: any[]; // 用户id集合
}
...
export interface UserLoginResp {
  ...
  orgIds?: any[]; // 用户所属组织id集合
  platTicket?: string;
  platformManager?: number; // 是否为平台管理员,0: 否 1: 是
  platformManagerPermissions?: any[]; // 平台管理员权限点列表
  signType?: string; // 签名类型
  signWay?: string; // 登录方式
  signatureImage?: string; // 签名照片url相对路径
  signatureImageWrite?: string; // 签名手写路径
...
export interface UserOfTenantDTO {
  ...
  tenantManagerPermissions?: any[]; // 租户管理员权限点列表
...
export interface WhereRelation {
  conditions?: any[]; // List对象可以是WhereItem,也可以是WhereRelation本身,实现递归
