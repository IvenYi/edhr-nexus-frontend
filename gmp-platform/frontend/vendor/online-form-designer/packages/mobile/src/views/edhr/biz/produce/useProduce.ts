...
async function loadQueryCards() {
  const clientKey = `${UserData.value?.userId}_searchtab_9384795024`;
...
async function loadQueryTotal(queryId: string) {
  ...
  const res = await postModelComprehensiveBizServiceByModelCategoryByModelKeyByBsKey(
    ...
    },
    {
      ...
      query: {
        ...queryTab._query,
        'f_task_type__jhwd.eq:t_szsyoshi': 'production',
      },
...
const onLoad = async () => {
  ...
  console.log('loadTaskCards onload');
