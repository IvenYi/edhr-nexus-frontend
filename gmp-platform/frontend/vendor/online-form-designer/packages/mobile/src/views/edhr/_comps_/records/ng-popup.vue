  ...
  >
    <div class="bg-white h-full flex flex-col">
      <div class="flex-none flex items-center pt-24px pb-4px pl-20px pr-20px">
        ...
        <span class="text-16px font-bold ml-8px">不良记录</span>
        ...
        <div class="p-20px">
          <TimelineList v-if="context.list.length > 0" :list="context.list">
            ...
            <template #default="{ record }: { record: IReportItem }">
              <div class="p-8px">
                <DescriptionItem name="不良数" :value="record.not_good_qty_" />
                <div class="bg-white p-8px mt-8px" v-if="record.entries_">
                  <DescriptionItem
                    v-for="(item, index) in record.entries_"
                    class="not-last-mb-8px"
                    :key="index"
                    :name="`${item.not_good_group_id_}/${item.not_good_reason_id_}`"
                    :value="item.not_good_qty_"
                  />
                </div>
              </div>
            </template>
