...
import ProduceReportPopup from '@mobile/views/edhr/_comps_/report/produce-report-popup.vue';
import { showSuccessToast } from 'vant';
import { useRoute, useRouter } from 'vue-router';
import ContainerOperationPicker from '@mobile/views/edhr/_comps_/container-operation/picker-popup.vue';
import dayjs from 'dayjs';
import type { IBtn } from '@mobile/views/edhr/_comps_/floating-btns/type.ts';
...
export interface IContainerOperation {
  ...
  status_: string;
  f_product_name_jhwd: string;
  product_version: string;
  current_routing_operation_names_: string;
}
  ...
  const floatingBtns = computed(() => {
    const actions: IBtn[] = [
      ...
      },
      {
        ...
        svg: ReportSvg,
        callback: () => {
          GctPopup.open(ProduceReportPopup, {
            context: {
              containerId: containerId.value,
              containerOperationId: containerOperationId.value,
              module: 'production',
        ...
        // 未开始
        hidden:
          containerOperationInfo.value?.report_enabled_ !== true ||
          containerOperationInfo.value?.status_ !== 'running',
      },
      {
        name: '返回列表',
        svg: ReturnSvg,
