...
<script setup lang="ts">
  ...
  async function loadGroups() {
    const res = await postModelDataQueryRefData({
      exp: '',
      query: {},
      modelKey: 'em_scrap_entry',
      fieldKey: 'scrap_group_id_',
      pageSize: 999,
      pageNo: 1,
      includeDeleted: false,
      refModelKey: 'em_scrap_group',
    });
    console.log(res);
  ...
  async function loadReasons(groupId: string) {
    const res = await postModelComprehensiveQueryRefChainDataByModelCategory(
      ...
      },
      {
        dataIds: groupId,
        modelKey: 'em_scrap_entry',
        fieldKey: 'scrap_reason_id_',
        refModelChain: [
          {
            id_: 'em_scrap_group$entries_',
            modelKey: 'em_scrap_group',
            modelCategory: 'entity',
            fieldKey: 'entries_',
            direction: 'forward',
          },
          {
            id_: 'em_scrap_group_entry$scrap_reason_id',
  ...
  const handleOk = async () => {
    ...
    if (props.onOk && typeof props.onOk === 'function') {
      ...
      if (ids.length === 1) {
        ngFormData.scrap_group_id_ = groupOption.key;
        ngFormData.scrap_group_name_ = groupOption.text;
        ngFormData.scrap_reason_id_ = undefined;
        ngFormData.scrap_reason_name_ = undefined;
      } else if (ids.length === 2) {
        ngFormData.scrap_group_id_ = groupOption.key;
        ngFormData.scrap_group_name_ = groupOption.text;
        const reasonOption = groupOption.children.find((item) => item.key === ids[1]);
        ngFormData.scrap_reason_id_ = reasonOption.key;
        ngFormData.scrap_reason_name_ = reasonOption.text;
      }
