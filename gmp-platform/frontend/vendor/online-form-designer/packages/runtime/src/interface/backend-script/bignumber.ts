declare module 'bignumber.js' {
  interface Config {
     ...
     * set({ DECIMAL_PLACES: 5 })
     * ```
     */
    DECIMAL_PLACES?: number;

    /**
     * An integer, 0 to 8. Default value: `ROUND_HALF_UP` (4).
     *
     ...
     *
     * ```ts
     * config({ EXPONENTIAL_AT: 2 })
     * new BigNumber(12.3)         // '12.3'        e is only 1
     * new BigNumber(123)          // '1.23e+2'
     * new BigNumber(0.123)        // '0.123'       e is only -1
     * new BigNumber(0.0123)       // '1.23e-2'
     *
     * config({ EXPONENTIAL_AT: [-7, 20] })
     * new BigNumber(123456789)    // '123456789'   e is only 8
     * new BigNumber(0.000000123)  // '1.23e-7'
     *
     * // Almost never return exponential notation:
     * config({ EXPONENTIAL_AT: 1e+9 })
     *
     ...
     * config({ CRYPTO: true })
     * config().CRYPTO       // true
     * random()              // 0.54340758610486147524
     * ```
     */
    CRYPTO?: boolean;

    /**
     * An integer, 0, 1, 3, 6 or 9. Default value: `ROUND_DOWN` (1).
     *
     ...
     *
     * ```ts
     * config({ MODULO_MODE: EUCLID })
     * set({ MODULO_MODE: 9 })          // equivalent
     * ```
     */
  ...
  class BigNumber implements Instance {
     ...
     * ```
     *
     * BigNumber `NaN` is returned if `n` is invalid (unless `DEBUG` is `true`, see below).
     *
     * ```ts
     * new BigNumber('.1*')                    // 'NaN'
     ...
     * ```
     *
     * To aid in debugging, if `DEBUG` is `true` then an error will be thrown on an
     ...
     * x.integerValue()                        // '123'
     * x.integerValue(ROUND_CEIL)    // '124'
     * y = new BigNumber(-12.7)
     * y.integerValue()                        // '-13'
     * x.integerValue(ROUND_DOWN)    // '-12'
     * ```
     *
     ...
     * x.precision(6, ROUND_UP)     // '9876.55'
     * x.precision(2)                         // '9900'
     ...
     * @param [decimalPlaces] Decimal places, integer, 0 to 1e+9.
     * @param [roundingMode] Rounding mode, integer, 0 to 8.
     * @param [format] Formatting options object. See `Format`.
     */
    ...
    /** Helps ES6 import. */
    private static readonly default?: Constructor;

    /** Helps ES6 import. */
    private static readonly BigNumber?: Constructor;
    ...
    /**
     * To aid in debugging, if a `DEBUG` property is `true` then an error will be thrown
     * if the BigNumber constructor receives an invalid `Value`, or if `isBigNumber`
     ...
     *
     * If `DEBUG` is `true`, throws if a BigNumber instance is not well-formed.
     *
     * ```ts
     * x = 42
     * y = new BigNumber(x)
     ...
     * maximum(4e9, x, '123456789.9')      // '4000000000'
     *
     * arr = [12, '13', new BigNumber(14)]
     * maximum.apply(null, arr)            // '14'
     * ```
     *
     * @param n A numeric value.
     */
     ...
     * ```ts
     * x = new BigNumber('3257869345.0378653')
     * min(4e9, x, '123456789.9')             // '123456789.9'
     *
     * arr = [2, new BigNumber(-14), '-15.9999', -12]
     * min.apply(null, arr)                   // '-15.9999'
     * ```
     *
     * @param n A numeric value.
     */
  ...
  function BigNumber(n: Value, base?: number): BigNumber;
