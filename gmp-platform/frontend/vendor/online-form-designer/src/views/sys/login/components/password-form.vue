<template>
  ...
  >
    ...
    <FormItem class="enter-x" v-if="isTestEnv && testTenantData?.name">
      <Input
        :title="testTenantData?.name"
        size="large"
        disabled
        :value="testTenantData?.name"
        class="fix-auto-fill"
      />
    </FormItem>
...
<script setup lang="ts">
  ...
  const { isTestEnv, testTenantData } = useEnv();
  ...
  async function handleLogin() {
    ...
    try {
      ...
      const userInfo = await userStore.login({
        password: sha256(data.password, key),
