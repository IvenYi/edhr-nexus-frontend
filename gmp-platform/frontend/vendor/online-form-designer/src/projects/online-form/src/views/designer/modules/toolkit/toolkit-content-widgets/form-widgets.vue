<template>
  <div class="wrapper">
    <div class="title">{{ $t('sys.onlineForm.formComponent') }}</div>
...
<script setup lang="ts" name="toolkit-content-reverse-modeling">
  ...
  import { OnlineFormNormalTypes } from '/@online-form/views/designer/constants';
  import {
    TransferType,
    setTransferData,
  ...
  const widgetConfigList = OnlineFormNormalTypes.map((type) => {
    return {
      type: type,
  ...
  function handleDragStart(e) {
    const source = e.target.closest('[data-type]');
