...
<script lang="ts" setup name="version-select">
  ...
  const ns = useNamespace('version-select');
  ...
  const keyField = computed(() => {
    return props.isRefId ? 'refId' : 'refId';
  });
...
<style lang="scss" scoped>
  ...
  @include b(version-select) {
    @include set-component-css-var(version-select, $version-select);
    height: getCssVar(version-select, height);
  }
</style>
