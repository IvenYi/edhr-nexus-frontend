<template>
  <div class="instance-content">
    <div class="instance-title-content">
      <div class="instance-title">{{ $t('sys.edhr.formInstList') }}</div>
    ...
    <div class="flex px16px py2px">
      <a-input
        ...
        :placeholder="$t('sys.onlineForm.pleaseEnterKeyword')"
        allowClear
          ...
          >
            <template #header>
              <div class="header-title-content" @click.stop="onSelectInstanceItem(item)">
                <span class="title" :title="calcTmplName(item, idx)">
                  {{ calcTmplName(item, idx) }}
                </span>
                <span v-if="supportEdit" class="hover-btn" @click.stop="onEdit(item)">
...
<script setup lang="ts">
  ...
  function calcTmplName(item: any, idx: number) {
    const newLabel = idx === 0 ? '(' + $t('sys.edhr.latest') + ')' : '';
    return (item.ext2 ?? item.tmplName ?? '') + newLabel;
  }
</script>
