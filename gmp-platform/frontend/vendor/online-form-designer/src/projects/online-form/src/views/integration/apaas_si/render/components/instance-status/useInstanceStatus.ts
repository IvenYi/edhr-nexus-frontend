...
export function useEnterFillProcess() {
  ...
  async function canEnterFillProcess(info) {
    ...
    if (
      ...
      message.warn(
        `当前DHR状态为${t(`sys.edhr.instanceStatus2DhrEnum.${instanceStatus}`)}，无法再进行填报`,
      );
      return false;
    }
    ...
    if (businessSetting.dhrSumDisabled === 1 && InstanceStatusValues.COMPLETED === instanceStatus) {
      ...
      if (
        ...
        message.warn(`当前${labelName[materialStatus]}${msg}，无法再进行填报`);
        return false;
      }
    }
  ...
  function canEnterChangeProcess(instanceStatus: InstanceStatusValue) {
    ...
    if (
      ...
      message.warn(
        `当前DHR状态为${t(`sys.edhr.instanceStatus2DhrEnum.${instanceStatus}`)}，无法进行变更`,
      );
      return false;
    }
