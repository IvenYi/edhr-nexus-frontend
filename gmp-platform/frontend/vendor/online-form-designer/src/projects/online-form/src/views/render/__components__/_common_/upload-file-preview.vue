<template>
  <div class="widget-file-form-wrapper">
    <div class="widget-file-form-container">
      ...
      >
        <a-upload-dragger
          class="widget-file-form-upload-dragger"
          :fileList="[]"
        ...
        >
          ...
          <p class="widget-file-form-text"> {{ $t('sys.edhr.uploadFileTips') }} </p>
          <p class="widget-file-form-hint">
            <!-- 支持上传扩展名为 .pdf 的文件，最多 {{ maxCount }} 个 -->
            {{ $t('sys.edhr.supportTypeAndMax', { type: '.pdf', max: maxCount }) }}
          </p>
        </a-upload-dragger>
      </div>
        ...
        <div class="pdf-viewport">
          ...
          <div class="pdf-sheet" :style="{ '--pad': '0px' }">
            ...
            <div v-if="!pdfLoaded" class="loading-overlay">
              <a-spin :tip="`${$t('sys.edhr.onLoadingFile')} PDF...`" />
            </div>

            <!-- 使用 vue-pdf-embed（或改回 iframe） -->
            <VuePdfEmbed
              v-if="currentFile.blobURL"
              :source="currentFile.blobURL"
...
<script setup lang="ts">
  ...
  import { getFileSize } from '/@/components/FieldUpload/src/hooks/hooks';
  ...
  async function handleUpload({ file }: { file: File }) {
    ...
    fileList.value.push({ uid: uuid2(16, 16), name: file.name });
    ...
    try {
      ...
      pdfLoaded.value = false;
      const remotePath = await Uploader.uploadByFile(file, true, props.modelKey);
      fileList.value[idx].path = remotePath;
      currentIndex.value = idx;
      await nextTick();
      value.value = fileList.value.map((f) => f.path || '');
    } catch (err: any) {
      fileList.value.splice(idx, 1);
      const errorMsg = err || $t('sys.onlineForm.unknownError');
      message.error(
        $t('sys.onlineForm.fileUploadError', {
          fileName: file.name,
          error: errorMsg,
        }),
      );
...
<style scoped lang="less">
  ...
  /* Upload 区域 */
  :deep(.widget-file-form-upload-dragger.ant-upload.ant-upload-drag) {
    background: #f7f8fa;
  }
  :deep(.widget-file-form-upload-dragger.ant-upload.ant-upload-drag .ant-upload) {
    padding: 40px 0;
  }
