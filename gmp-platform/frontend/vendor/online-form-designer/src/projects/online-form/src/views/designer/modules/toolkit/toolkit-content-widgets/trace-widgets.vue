<template>
  <div class="wrapper">
    <div class="title">{{ $t('sys.onlineForm.traceabilityComponent') }}</div>
...
<script setup lang="ts" name="toolkit-content-reverse-modeling">
  ...
  import { OnlineFormTraceTypes } from '/@online-form/views/designer/constants';
  import {
    TransferType,
    setTransferData,
  ...
  const widgetConfigList = OnlineFormTraceTypes.map((type) => {
    return {
      type: type,
