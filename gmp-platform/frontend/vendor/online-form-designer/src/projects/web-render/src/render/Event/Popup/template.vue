<template>
  ...
  >
    <div class="popupheaher ks-row-middle">
      ...
      <div class="w30px text-left" @click.stop="close"> <van-icon name="cross" /></div>
    </div>
...
<script setup lang="ts">
  ...
  const popStyle = computed(() => {
    return {
      height: `${modalProps.mModalWidth}${modalProps.mUnitType}`,
      width: '100%',
    };
  });
  ...
  async function open({ title, onOpen, onClose, data }: any = {}) {
    ...
    modalProps.mUnitType = get(props, 'mUnitType', '%');
    modalProps.mModalWidth = !has(props, 'mModalWidth')
      ? get(props, 'modalWidthPercent', 70)
      : get(props, 'mModalWidth', 60);
