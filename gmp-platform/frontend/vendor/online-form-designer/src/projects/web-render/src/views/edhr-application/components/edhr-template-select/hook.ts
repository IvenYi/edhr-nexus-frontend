...
export function useAsyncOptions() {
          ...
          return {
            ...
            title: () =>
              h('div', [
                ...
                j.default_ ? h('span', { class: 'version gct-custom-tag ml8px' }, '默认') : null,
              ]),
            name: i.name_,
            default_: j.default_,
            _info: { ...j },
  ...
  async function getChildrens({ id_ } = {}) {
    ...
    const { data } =
      (await getEdhrTmplGetVersionById({
        id: id_,
      })) || {};
    // 数据转化为 rdo 结构
    ...
    try {
      const chl = (await getEdhrTmplListVersionById({ id: _fId })) || {};
