  ...
  <div class="ks-column h100%">
    <div class="ks-col export-container">
      ...
      <div v-if="loading" class="text-[#8f8f8f]">
        {{ t('sys.component.upload.exportCloseModalTip') }}
      </div>
      ...
      <div v-if="isComplete && !!data.exportInfo?.length" style="margin-top: 64px">
        <div>{{ t('sys.pageDesigner.exportDetail') }}：</div>
        <div v-for="(item, index) in data.exportInfo" :key="index">
          <div style="color: #8f8f8f">
            {{ index + 1 }}.【{{ item.name }}】 {{ t('sys.component.upload.exportTotalData', [item.succeed]) }}
          </div>
        </div>
      </div>
    </div>
