  ...
  <div class="process-wrap ks-column overflow-hidden">
    <a-collapse v-model:activeKey="activeKey" ghost expand-icon-position="right">
      <a-collapse-panel key="1">
        <template #header>
          <div class="title">{{ $t('sys.edhr.processInfo') }}</div>
        </template>
        <div class="rounded-8px bg-[#FAFAFA] px16px pt16px">
          ...
          >
            <a-descriptions-item :label="$t('sys.process.name')">
              {{ processInfo.name_ }}
            </a-descriptions-item>
            <a-descriptions-item :label="$t('sys.createTime')">
              {{ processInfo.create_time_ }}
            </a-descriptions-item>
            <a-descriptions-item :label="$t('sys.edhr.processDesc')" :span="2">
...
<script setup lang="ts">
  import FormPRocessPath from '@/components/BpmnRuntime/form/index.vue';
