<template>
  <div :class="[ns.b(), 'approval-task-modal']">
    ...
    <EdhrContent
      ...
      :edhr-id="dhrId"
      :readonly="true"
      :requestOutLine="getCacheOutLineData"
    />
...
<script setup lang="ts">
  ...
  const ns = useNamespace('dhr-temp-info');
  ...
  const dhrId = computed(() => {
    if (outLineInfo.value && outLineInfo.value.length) {
      return outLineInfo.value[0].parentId;
    }
    return null;
  });

  async function getCacheOutLineData() {
    await new Promise((resolve) => setTimeout(resolve, 200));
    console.log('getCacheOutLineData: outLineInfo.value', outLineInfo.value);
    return outLineInfo.value;
  }
...
<style lang="scss" scoped>
  ...
  @include b(dhr-temp-info) {
    @include set-component-css-var(dhr-temp-info, $dhr-temp-info);
