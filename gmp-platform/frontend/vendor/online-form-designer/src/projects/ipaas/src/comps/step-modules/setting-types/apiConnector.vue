<template>
  <div>
    ...
    <a-form ref="formRef" :model="formState" autocomplete="off" layout="vertical">
      ...
      <a-form-item
        v-if="formState.httpConfig"
        :label="$t('sys.ipaas.domainConfig')"
        :name="['httpConfig', 'host']"
        :rules="[{ required: true }]"
      >
        <a-input-group>
          <a-row>
            <a-col :span="8">
              <a-form-item :name="['httpConfig', 'protocol']" :rules="[{ required: true }]">
                <a-select
                  v-model:value="formState.httpConfig.protocol"
                  size="small"
                  :disabled="readonly"
                  style="width: 100%"
                >
                  <a-select-option v-for="item in Object.values(ProtocolTypes)" :key="item">
                    {{ $t(`sys.ipaas.ProtocolTypes.${item}`) }}
                  </a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="16">
              <a-form-item :name="['httpConfig', 'host']" :rules="[{ required: true }]">
                <a-input
                  v-model:value="formState.httpConfig.host"
                  size="small"
      ...
      <!-- <a-form-item label="超时时间（秒）" name="connectTimeOut">
        <a-input-number
          ...
          :disabled="readonly"
        />
        <div class="text-[#C3C3C3] text-12px mt2px font-300">{{ '不填默认60秒' }}</div>
      </a-form-item> -->
...
<script setup lang="ts">
  ...
  import { HttpMethod, ParamType, ProtocolTypes } from '@gct/flow/src/plugins/ipaas-bpmn/enums';
  ...
  const formState = reactive<NodeBizDataSchema.ApiConnector['nodeConfig']>(
