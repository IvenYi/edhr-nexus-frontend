...
interface OrderPackageRuleEntry extends CommonFields {
  /**
   * 包装方式
   *
   * @author zyl
...
sort_num_: number,
   ...
   * @type {string}
   */
package_bar_code_entries: string,
...
/**
 *模型名称：工单包装规则配置
 *模型KEY:em_order_package_rule_entry
 */
interface OrderPackageRuleEntryMethods extends IModelService<OrderPackageRuleEntry> {
  /**
   * 规则明细是否配置了允许混工单
   *
   ...
   * @return boolean
   */
isAllowMixMfgOrder(detailId:string):boolean;
   ...
   * @return boolean
   */
isAllowMixProduct(detailId:string):boolean;
   ...
   * @return boolean
   */
isAllowNotFullBox(detailId:string):boolean;


  /**
   * 规则明细是否配置了验证批次完成
   *
   * @param1 detailId 工单包装规则配置id
   * @return boolean
   */
isValidateContainerFinished(detailId:string):boolean;
