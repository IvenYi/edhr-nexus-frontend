<template>
  <a-collapse :activeKey="activeNames" ghost class="gct-vant-item">
    <template #expandIcon>
      <down-outlined class="collapse-icon-down" :style="`margin-right: ${style.paddingRight}px;`" />
    </template>
    <a-collapse-panel
      key="1"
      ...
      <template #header>
        <a-row class="w100%" type="flex" align="middle" :style="headerStyle">
          ...
          <a-col
            ...
            :style="{
              minHeight: '32px',
              backgroundColor: !widget.children[1]?.children.length ? '#FAFAFA' : '',
            }"
          >
            <drag-widget-group
...
<script setup lang="ts" name="gct-collapse">
  ...
  const titleStyleAttr = computed(() => {
    return {
      ...
      color: style.value.labelFont?.color || 'rgba(0,0,0, .85)',
