  ...
  >
    <template #input>
      <div class="text-[14px] w-full">
        ...
        <div
          class="mt8px overflow-y-auto"
          :style="{
            height: widget.style.height ? widget.style.height + 'px' : 'auto',
            maxHeight: widget.style.height ? widget.style.height + 'px' : '320px',
          }"
        >
          <van-row>
            <van-col :span="8" v-for="(item, index) in fileArr" :key="index">
              <div class="progress-wrap mb12px mr12px p12px pr0">
  ...
  watch(
    ...
    async () => {
      const ids = value.value.map((i) => {
        return i.split('/')[i.split('/').length - 2];
      });
