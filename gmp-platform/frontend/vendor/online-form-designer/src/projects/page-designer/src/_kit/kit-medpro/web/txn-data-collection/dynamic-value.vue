<template>
  <a-form-item
    ...
    style="width: 100%"
    :class="readonly ? 'dynamic-value-wrap' : ''"
  >
    <image-upload
    ...
    />
    <component
      v-else-if="[FormComponents.Userpicker, FormComponents.Department].includes(widgetType)"
      :is="defComponet"
    ...
    <div v-else-if="widgetType === FormComponents.Inputnumber">
      <div
        ...
        v-if="showLimit && (formData.validate_range_ || formData.remind_enabled_)"
      >
        <span
          v-if="max !== undefined"
      ...
      <a-input-number
        :precision="
          formData.type_ === 'integer'
            ...
            : undefined
        "
        :min="formData.auto_fix_ ? min : undefined"
        :max="formData.auto_fix_ ? max : undefined"
        v-model:value="value"
      />
    </div>
    <a-date-picker
...
<script setup lang="ts">
  ...
  const showLimit = computed(() => {
    return formData.value.show_limit_ === undefined ? true : formData.value.show_limit_;
  });
  ...
  const defComponet = computed(() => {
    ...
    return AsyncGctComponents.getComponentByType(widgetSchema.value.type);
  });
  ...
  const value = computed<any>({
    ...
    set(v) {
      ...
      // 精度小数补齐小数位
      // if (formData.value.type_ == FIELD_TYPE.DECIMAL) {
      //   const digits_ = formData.value.digits_;
      //   if (isNumber(digits_)) {
      //     v = isNil(v) || v === '' ? v : Number(v).toFixed(digits_);
      //   }
      // }
  ...
  async function validateRequire() {
    await nextTick();
    const { name_ } = formData.value;
    if (
      (value.value === '' || value.value === undefined || value.value === null) &&
      value.value !== 0
    ) {
      return Promise.reject(t('sys.pageDesigner.sthCannotBeEmpty', { sth: name_ }));
    }
  }
  ...
  function formateReadonlyValue(val) {
    ...
    if (type_ === 'boolean') {
      return val?.toString() === 'true'
        ? formData.value.true_text_ || '真'
        : formData.value.false_text_ || '假';
    }
    return value.value;
  }
...
<style lang="less" scoped>
  ...
  .ant-form-item.dynamic-value-wrap {
    margin-bottom: 0;
  }
</style>
