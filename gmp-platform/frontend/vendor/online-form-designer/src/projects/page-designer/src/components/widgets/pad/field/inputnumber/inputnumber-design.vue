<template>
  ...
  >
    <template #input>
      <div>
        {{ bindCompStyleType === BindCmpStyleEnum.CMP_CURRENCY ? currency : '' }}
        <span class="text-[#C6C6C6]">{{ placeholder }}</span>
      </div>
    </template>
  </van-field>
</template>
...
<script setup lang="ts" name="gct-inputnumber">
  ...
  const { fieldType, readonly, bindCompStyleType, currency, displayTimeType, placeholder } = toRefs(
    props.widget.props,
  );
