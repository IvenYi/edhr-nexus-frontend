<template>
  <div v-if="readonly">
    ...
    />
    <span
      v-show="fieldObj?.name && !fieldObj?.version && !getIsBtwLabel(fieldObj)"
...
<script name="gct-document-template" setup lang="ts">
  ...
  import labelTmplModal from '../../__components__/print-tmpl-modal.vue';
  import {
    getPrintDesignerInfo,
    // getPrintDesignerRdoPageList,
  } from '/@/apis/gct-apaas/PrintDesignerController';
  import { ILabelTemplateRefComponentExpose } from '/@/projects/page-designer/src/interface/web';
  ...
  const optionsData = computed(() => {
    const data = options.value.map((e) => {
      return {
        ...e,
        fieldLabel: `${e.name}${e.version && e.printType !== 'btw' ? ' : ' + e.version : ''}`,
        showTitle: h('div', [
          h(taglabel, {
  ...
  watch(
    ...
    async () => {
      if (value.value) {
        ...
        fieldObj.value = {
          ...data,
          version: getIsBtwLabel(data) ? '' : data.version, // 手动清空
        };
      } else fieldObj.value = {};
    },
    {
  ...
  //   categoryList.value = (await getCategoryList({ module: PrintTypeEnum.LABEL as string })) || [];
  //   selectCategoryId.value = categoryList.value[0].id;
  // }

  // async function getModelList() {
  //   const treeData: any =
  //     (await getPrintDesignerRdoPageList({
  //       moduleType: PrintTypeEnum.LABEL as string,
  //       pageNo: 1,
  //       pageSize: 20,
  //       categoryId: selectCategoryId.value,
  //     })) || {};
  //   treeFlat.value = tree2list(treeData.data || []);
  // }

  // async function getModelList() {
  //   treeData.value =
  //     (await getCategoryListComplete({ module: PrintTypeEnum.LABEL as string })) || [];
  //   treeFlat.value = tree2list(treeData.value);
  // }
  ...
  defineExpose<ILabelTemplateRefComponentExpose>({ getValue, setValue });
</script>
