<template>
  ...
  >
    ...
    >
      <div class="pitch-on">
        <div v-if="checkboxRow.length > 0" class="flex items-center">
          ...
          >
            <i class="gct-iconfont icon-guanbi-danchuang"></i>
          </span>
        </div>
...
<script setup lang="ts" name="gct-data-table">
  import { ref, reactive, toRaw, onMounted, computed, toRef, nextTick } from 'vue';
  ...
  import { vxeRefTable, RenderTableButtons, useTableLayout } from './component/vxeRenderTable';
  ...
  const maxHeight = props.widget.style.maxHeight || 600;
  const pageSize = 20;
  const {
    refSearch,
    model,
  ...
  async function getBodyBySearchComponent(key: string) {
    ...
    try {
      /**重查询组件内获取查询条件 */
      let searchVm = (await Event.getComponent(key)) as any;
