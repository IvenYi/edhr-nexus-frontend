<template>
  ...
  >
    <div v-if="showFields?.length">
      ...
      >
        <van-icon name="arrow-up" color="#ffffff" v-if="showMore" />
        <van-icon name="arrow-down" color="#ffffff" v-else />
      </div>
    </div>
  </div>
</template>
...
<script lang="ts" setup name="gct-container-search-design">
  ...
  const { placeholder, scanSite, maxLength, rowLength } = toRefs(defProps.widget.props);
  ...
  const styleWrap = computed(() => {
    ...
    const { enableBGColor, backgroundColor } = style as any;
    const bgColor = enableBGColor ?  'var(--van-primary-color)' : (backgroundColor || 'var(--van-primary-color)');
    return {
