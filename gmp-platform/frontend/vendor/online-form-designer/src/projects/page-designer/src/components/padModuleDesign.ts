...
export class AsyncGctComponents {
  ...
  static init() {
    ...
    console.log(AsyncGctComponents.componentMap);
    // if (window._gct) {
    //   _gct.register.designer.pad.hooks.register.tap((_, tag, designer) => {
    //     if (designer) {
    //       AsyncGctComponents.componentMap.set('gct-plugin-' + tag, designer.component as any);
    //     }
    //   });
    //   const keys2 = _gct.register.designer.pad.getKeys();
    //   keys2.forEach((key) => {
    //     const provider = _gct.register.designer.pad.getProvider(key);
    //     if (provider) {
