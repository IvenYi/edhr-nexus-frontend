...
<script setup lang="ts">
  ...
  const BusinessButtonComponents = [
    MedProKitButtonType.CREATE,
    MedProKitButtonType.COPY,
    MedProKitButtonType.COPYVERSION,
    MedProKitButtonType.DELETE,
    MedProKitButtonType.MODELING,
    MedProKitButtonType.USEINFO,
  ];
  ...
  const buttonOptions = computed(() => {
    return BusinessButtonComponents.map(createWidgetByType);
  });
  ...
  const open = async (t: string, modalFormData: any): Promise<typeof formState.value> => {
    formState.value.name = '';
    formState.value.type = modalFormData || [];
