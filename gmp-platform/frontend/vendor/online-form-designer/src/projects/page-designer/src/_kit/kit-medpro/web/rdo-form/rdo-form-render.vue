...
<script setup lang="ts" name="gct-rdo-form">
  ...
  import { ref, reactive, watch, provide, toRef, onMounted } from 'vue';
  ...
  import { ResetRuleType, FIELD_TYPE } from '@gct/runtime';
  ...
  onMounted(async () => {
    ...
    if (model) {
      try {
        fieldMetaList.value = (await getFieldMetaList({ modelKey: model })) || [];
      } catch (error) {
  ...
  async function submit() {
    ...
    if (id_ && !_isCreated.value) {
      return await uploadForm('rdoUpdateVersionById', id_);
    } else if (base_id_) {
      return await saveForm('rdoSaveVersion');
    } else {
      return await saveForm('rdoSave');
    }
  }
  ...
  async function reset(type: ResetRuleType = ResetRuleType.WIPE_DATA) {
    if (type === ResetRuleType.REFRESHDATA && formState.id_) {
      ...
      const name_ = _disabledName.value ? formState[rdoUniqueFieldKey] : '';
      setValue({ [rdoUniqueFieldKey]: name_, id_: formState.id_ });
      await clearValidate();
      forEachKey.value = Math.random() + '';
    }
  }
   ...
   * 清空序列号字段的值
   */
  function clearSerialNumberFields(data: any) {
    const serialFields = fieldMetaList.value.filter((field) => field.type === FIELD_TYPE.SERIAL);
  ...
  defineExpose({
    ...
    copyData(data = formState) {
      const name_ = data[rdoUniqueFieldKey];
      const copiedData = {
        ...data,
        [rdoUniqueFieldKey]: name_ ? `copy_of_${name_}` : null,
        base_id_: null,
        default_: false,
      };
      // 清空序列号字段
      clearSerialNumberFields(copiedData);
      setValue!(copiedData);
    ...
    createVersion(data = formState) {
      const { id_, base_id_ } = data;
      setValue!({ [rdoUniqueFieldKey]: data[rdoUniqueFieldKey], base_id_: base_id_ || id_ });
      _isCreated.value = true;
      _disabledName.value = true;
    },
  });
