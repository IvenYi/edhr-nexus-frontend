      ...
      <a-row :gutter="10">
        <a-col :span="19">
          ...
          <a-select
            ...
            option-label-prop="children"
            style="width: 100%"
            @change="handleChangeTagType"
          >
            <a-select-option :value="i" v-for="i in TagTypeEnum" :key="i">
              <div class="tag-opt">
                <div class="tag-icon" :class="i"></div>{{ $t(`sys.pageDesigner.${i}`) }}
              </div>
            </a-select-option>
          </a-select>
        </a-col>
...
<script setup lang="ts">
  ...
  const props = defineProps({
    ...
    tagStyle: {
      type: Object as PropType<LowCodeWidget.TagConfigStyle>,
      default: () => ({
        color: '',
    ...
    tagType: {
      type: String as PropType<tagEnum>,
      default: tagEnum.TAG,
    },
  });
  ...
  const handleUpdateColor = (_e, color) => {
    ...
    if (!props.widget?.style.tagStyle) {
      props.widget!.style.tagStyle! = { color: tagStyleRef.value.color };
    } else {
      props.widget!.style.tagStyle.color = tagStyleRef.value.color;
    }
  };
  const handleChangeTagType = () => {
    if (!props.widget?.style.tagStyle) {
      props.widget!.style.tagStyle! = { tagType: tagStyleRef.value.tagType };
    } else {
      props.widget!.style.tagStyle.tagType = tagStyleRef.value.tagType;
    }
  };
