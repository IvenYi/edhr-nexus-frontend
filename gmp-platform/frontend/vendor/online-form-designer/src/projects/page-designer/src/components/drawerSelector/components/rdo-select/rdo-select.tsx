...
import './rdo-select.scss';
import rdoTable from './rdo-table.vue';

export const RdoSelect = defineComponent({
  name: 'RdoSelect',
  components: {
    'van-list': List,
    rdoTable,
  ...
  setup(props, { emit }) {
    ...
    const handleClick = (value: string, option: Option): void => {
      if (props.refVersion && option.children && option.children.length) {
    ...
    const renderChildItem = (child: Option) => {
      const isDefault = child._protoValue?.default_;
    ...
    return () => {
      return (
        <div class={ns.b()}>
          {props.displayFields && props.displayFields.length <= 1 && renderRdoList()}
          {props.displayFields && props.displayFields.length > 1 && renderRdoTable()}
          {/* 无数据状态 */}
          {!props.options.length &&
            !props.isLoading &&
            props.displayFields?.length == 1 &&
            renderEmpty()}
        </div>
      );
    };
