...
import './vue3-dnd-highlighter-container.scss';
...
export const Vue3DndHighlighterContainer = defineComponent({
  name: 'Vue3DndHighlighterContainer',
  props: {
    mode: {
  ...
  setup(props) {
    const ns = useNamespace('vue3-dnd-highlighter-container');
    ...
    const styleObj = computed(() => {
      const style = {
        'background-color': c.state.isDragging || props.mode === 'select' ? 'transparent' : '',
  ...
  render() {
    return (
      <div style={this.styleObj} class={[this.ns.b(), this.ns.m(this.mode)]}>
        <div class={[this.ns.e('mask'), this.ns.m(this.mode)]}></div>
      </div>
    );
  },
});
