<template>
  <div class="w-full" style="width: 100%">
    <div
      class="button-group-wrap py-16px px-24px"
      :class="buttonGroup.length == 1 ? 'flex-start' : ''"
    >
      <template v-for="item in btnGroupList" :key="item.type">
        <div
          v-if="buttonGroup.includes(item.type)"
          @click.stop="handleClick(item.type)"
          class="btn-item"
        >
          <div class="loading" v-if="loading(item.type)"
            ><loading-outlined class="loading-icon"
          /></div>
          <SvgIcon
            :class="['btn-icon', setBtnColor(item.type) ? 'btn-color' : '']"
            size="40"
            :name="`btn-${item.type}`"
          />
          <span :class="['btn-name', loading(item.type) ? 'disabled' : '']">{{
            $t(item.name)
          }}</span>
        </div>
      </template>
    </div>
  </div>
</template>

<script setup lang="ts" name="gct-button-group">
  import { computed, ref, reactive } from 'vue';
  import type { IButtonGroup } from './schema';
  import { SvgIcon } from '/@/components/Icon';
  import { btnGroupType, btnGroupData } from './type';
  import { getPageEvent } from '/@page-designer/components/widgets/hooks/hooks';
  import { Modal, message } from 'ant-design-vue';
  import { useI18n } from '/@/hooks/web/useI18n';

  const props = defineProps<{ widget: IButtonGroup }>();
  const { refForm, buttonGroup, model } = reactive(props.widget.props);
  const Event = getPageEvent();
  const { t } = useI18n();

  const loadingMap = ref(new Map());

  const btnGroupList = computed(() => {
    return btnGroupData.filter((i) => buttonGroup.includes(i.type));
  });

  const loading = computed(() => {
    return (type: btnGroupType) => {
      return loadingMap.value.get(type);
    };
  });

  const setBtnColor = computed(() => {
    return (type) => {
      return [btnGroupType.MODELING, btnGroupType.COPY].includes(type);
    };
  });

  const handleClick = async (val: btnGroupType) => {
    if (loadingMap.value.get(val)) return;
    switch (val) {
      case btnGroupType.CREATE:
        handleCreate(val);
        break;
      case btnGroupType.COPY:
        handleCopy(val);
        break;
      case btnGroupType.COPYVERSION:
        copyVersion(val);
        break;
      case btnGroupType.DELETE:
        handleDelete(val);
        break;
      case btnGroupType.MODELING:
        handleModeling();
        break;
      case btnGroupType.USEINFO:
        handleUseInfo();
        break;
    }
    await Event.runEventByName('onClick', props.widget.events, val);
  };

  const handleCreate = async (type: btnGroupType) => {
    const form = await Event.getSyncComponent(refForm);
    form.createVersion();
    // FIX: 业务按钮点击，页面绑定事件后会执行对应事件导致操作流程不正确
    // await Event.runEventByName('afterExecution', props.widget.events, type);
  };

  const handleCopy = async (type: btnGroupType) => {
    const form = await Event.getSyncComponent(refForm);
    form.copyData();
    // FIX: 业务按钮点击，页面绑定事件后会执行对应事件导致操作流程不正确
    // await Event.runEventByName('afterExecution', props.widget.events, type);
  };

  const copyVersion = async (type: btnGroupType) => {
    const form = await Event.getSyncComponent(refForm);
    form.copyVersion();
    // FIX: 业务按钮点击，页面绑定事件后会执行对应事件导致操作流程不正确
    // await Event.runEventByName('afterExecution', props.widget.events, type);
  };

  const handleDelete = async (type: btnGroupType) => {
    try {
      Modal.confirm({
        title: t('sys.confirmExecution'),
        okText: t('sys.ok'),
        cancelText: t('sys.cancel'),
        async onOk() {
          loadingMap.value.set('delete', true);
          const form = await Event.getSyncComponent(refForm);
          await form.deleteData();
          message.success(t('sys.delSuccess'));
          await Event.runEventByName('afterExecution', props.widget.events, type);
        },
        onCancel() {},
      });
    } catch (error) {
      console.error(error);
    }
    loadingMap.value.set('delete', false);
  };

  const handleModeling = async () => {
    try {
      const form = await Event.getSyncComponent(refForm);
      let id = form.getValue!().id_;
      if (!id) return;
      loadingMap.value.set('modeling', true);
      await Event.context.$modelingTraceability!({ id, modelKey: model }).open();
    } catch (error) {
      console.log(error);
    }
    loadingMap.value.set('modeling', false);
  };

  const handleUseInfo = async () => {
    try {
      loadingMap.value.set('useInfo', true);
      const form = await Event.getSyncComponent(refForm);
      const id = form.getValue!().id_;
      await Event.context.$usageInformation!({
        id,
        row: form.getValue!(),
        modelKey: props.widget.props.bindModelKey,
      });
    } catch (error) {
      console.log(error);
    }
    loadingMap.value.set('useInfo', false);
  };
</script>

<style scoped lang="less">
  .button-group-wrap {
    display: flex;
    justify-content: space-around;
    border-radius: 4px;
    background: #fafafa;

    &.flex-start {
      justify-content: flex-start;
    }

    .btn-item {
      position: relative;
      cursor: pointer;

      .btn-name {
        &.disabled {
          color: #ccc;
        }
      }

      &:hover {
        .btn-name {
          color: var(--ant-primary-color);

          &.disabled {
            color: #ccc;
          }
        }
      }

      .loading {
        position: absolute;
        left: 50%;
        width: 40px;
        height: 40px;
        padding: 0 5px;
        transform: translate(-50%, 0);
        background: rgb(255 255 255 / 50%);
        color: var(--ant-primary-color);
        font-size: 30px;

        .loading-icon {
          position: absolute;
          top: 5px;
        }
      }

      .btn-icon {
        display: block;
        margin: 0 auto;
      }

      .btn-color {
        color: #ff914a;
      }

      .btn-name {
        display: block;
        margin-top: 8px;
        line-height: 20px;
        text-align: center;
      }
    }
  }
</style>
