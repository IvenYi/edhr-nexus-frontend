  ...
  >
    ...
    <template #default="{ column: { params, treeNode }, row }">
      <div class="ks-row-middle">
        ...
        <i
          class="gct-iconfont icon-icon_hangbianji ml5px text-[#808080]"
          v-if="
            isClickEdit &&
    ...
    <template #ope_header v-if="operateColumn">
      ...
      ></i> -->
      <div class="operate-column-header pl10px pr10px pb6px pt6px">
        {{ operateColumn!.props.label }}</div
      >
    </template>
    <template v-if="operateColumn" #ope_default="{ row, rowIndex }">
      <div class="operate-column-cell">
...
<script setup lang="tsx">
  ...
  function transformFont2flexStyle(params) {
    const flexStyle = params.style?.contentFont?.align;
    return alignMap[flexStyle] || 'flex-start';
  }

  function getCellStyle(params) {
    return {
      width: !params.props.readonly ? 'calc(100% - 22px)' : '100%',
      display: 'flex',
      justifyContent: transformFont2flexStyle(params),
      // fontSize: (params.style?.contentFont?.fontSize || 16) + 'px',
    };
  }
...
<style lang="less">
  ...
  .vxetable {
    ...
    .vxe-body--column.col--seq > .vxe-cell {
      font-size: 16px;
    }

    // 只在操作列应用 padding
    .vxe-header--column:has(.operate-column-header),
    .vxe-body--column:has(.operate-column-cell) {
