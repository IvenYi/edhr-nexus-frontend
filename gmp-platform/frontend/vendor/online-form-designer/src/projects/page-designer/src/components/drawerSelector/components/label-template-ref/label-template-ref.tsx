...
import './label-template-ref.scss';
...
export const LabelSelect = defineComponent({
  ...
  setup(props, { emit }) {
      ...
      if (existingOptionValues.value.size === 0) {
        ...
        options.forEach((option) => {
          if (option.children && option.children.length > 0) {
            ...
            option.children.forEach((p) => {
              if (p.children && p.children.length > 0) {
                expandedItems.value.add(p.value as string);
              }
            });
          }
        });
        ...
        <div key={option.value} class={ns.e('parent-item')}>
          ...
          >
            ...
            {option.children && option.children.length ? (
              ...
              <div class={ns.e('expand-area')}></div>
            )}
    ...
    const renderChildItem = (child: Option) => {
      const isDefault = child._protoValue?.defaulted;
      const isExpanded = expandedItems.value.has(child.value as string);
      return (
        <div>
          <div
            key={child.value}
          ...
          >
            ...
            {child.children && child.children.length ? (
              <div
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  toggleExpanded(child.value as string);
                }}
              >
                <i
                  class={[
          ...
          {/* 子节点列表 */}
          {isExpanded && (
            <div class={ns.e('sun-list')}>
              {child.children!.map((child) => renderChildItem(child))}
            </div>
          )}
        </div>
      );
