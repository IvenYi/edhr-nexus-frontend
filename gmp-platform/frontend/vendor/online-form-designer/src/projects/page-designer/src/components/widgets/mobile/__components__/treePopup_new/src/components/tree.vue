...
<script setup name="treeItem" lang="ts">
  ...
  const treeChecked: any = ref(props.options?.map((item) => item.checked) || []);
  const itemHeight: any = ref(props.options?.map((i) => ''));
  ...
  watch(
    ...
    () => {
      ...
      treeChecked.value = props.options?.map((item) => item.checked) || [];
    },
    { deep: true },
  );
  ...
  function checked(value: any, index: any) {
    const checked = !props.multiple ? true : treeChecked.value[index];
