import { IVTableActionItem, IVTableDataItem } from '@gct/universal-component/gct-v-table';
import { Events } from '/@web-render/render/Event/baseEvent';
import { showConfirmDialog } from 'vant';

export class SubVTableBaseAction {
  ...
  protected hidden(row: IVTableDataItem, rowIndex?: number): boolean {
    // 做按钮隐藏判断，主要用于子类重写和继承
  ...
  protected async onAction(row: IVTableDataItem, rowIndex?: number): Promise<void> {
  ...
  protected async onClick(row: IVTableDataItem, rowIndex?: number): Promise<void> {
    // 根据具体的子类实现不同的点击事件
  }
