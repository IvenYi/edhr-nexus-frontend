...
<script setup lang="ts" name="var-modal">
  ...
  const { keyPrefix, keySuffix, keyPad, keyClip, keyReset } = useKeyParser('$PAGERVAR_');
  ...
  const onDataReceive = (data) => {
    ...
    const varInfo = Object.assign(cloneDeep(data.varInfo), { key });
    if (
  ...
  const handleOk = () => {
    varFormRef.value!.validate().then(() => {
      const isSameName = pageJson.pageVars.some((n) => {
        return n.key === keyPad(formState.value.varInfo.key) && n.id !== formState.value.id;
      });
      if (isSameName) {
        message.error(t('sys.pageDesigner.varNameExisted'));
        return;
      }
