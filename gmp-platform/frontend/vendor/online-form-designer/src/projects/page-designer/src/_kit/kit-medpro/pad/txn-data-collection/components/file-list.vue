<template>
  <div class="file-list-wrap">
    <div class="file-list-wrap__list flex-card" v-if="tableData.length">
      <div
        v-for="(file, index) of tableData"
        :key="index"
        :class="['file-list-wrap__item', isDesign ? 'is-design' : '']"
        @click.stop="handleClick(file)"
      >
        <span :class="['file-status', file.status]">{{ StatusNames[file.status] }}</span>
        <IconNext
          class="file-list-wrap__item-svg"
          :size="34"
          :value="'icon-preset:' + typeParser(file)"
        />

        <div class="file-list-wrap__item-name">
          <span :title="isDesign ? '' : file.name">
            {{ file.snContainerName ? `【${file.snContainerName}】${file.name}` : file.name }}
          </span>
        </div>
      </div>
    </div>
    <van-empty description="暂无数据" v-else />
  </div>
</template>

<script setup lang="ts" name="fileList">
  import { computed } from 'vue';
  import { CollectionData, typeParser, StatusNames } from '../types';
  import IconNext from '/@/components/Icon/src/IconNext.vue';

  interface IProps {
    fileData: CollectionData[];
    isDesign: boolean;
    fileLoading?: boolean;
  }

  const props = defineProps<IProps>();
  const emit = defineEmits(['fileCheck']);

  const tableData = computed(() => {
    return props.fileData.map((i) => i);
  });

  async function handleClick(item) {
    if (props.isDesign) return;
    emit('fileCheck', item);
  }
</script>

<style lang="less" scoped>
  .file-list-wrap {
    // padding-top: 8px;

    &__list {
      display: flex;
      &::-webkit-scrollbar {
        display: block;
        width: 4px;
      }
      &.flex-card {
        flex-direction: row;
        flex-wrap: wrap;
        max-height: 372px;
        overflow-y: auto;
        .file-list-wrap__item {
          position: relative;
          padding-top: 16px;
          text-align: center;
          width: 130px;
          margin-right: 10px;
          padding: 8px;
          color: #000;
          background: #fff;
          border: 1px solid #e8ebf0;
          border-radius: 4px 4px 4px 4px;
          &-svg {
            margin: 0 auto;
            margin-top: 8px;
          }
          &-name {
            width: 100%;
            margin-top: 2px;
            height: 22px;
            line-height: 22px;
          }
          .file-status {
            position: absolute;
            right: 0;
            top: 0;
            line-height: 12px;
            border-radius: 0 4px 0 4px;
          }
          &:hover {
            border-color: #026ac8;
          }
        }
      }
    }

    &__item {
      margin-bottom: 8px;
      &-name {
        span {
          display: inline-block;
          max-width: 100%;
          white-space: nowrap;
          overflow: hidden;
          text-overflow: ellipsis;
        }
      }
      &:hover {
        cursor: pointer;
        color: var(--ant-primary-color);
      }
      &.is-design {
        cursor: default;
        color: rgba(0, 0, 0, 0.85);
      }
    }
  }

  .file-status {
    font-size: 12px;
    padding: 2px 4px;
    &.initial {
      color: #026ac8;
      background: rgba(2, 106, 200, 0.1);
    }
    &.stash {
      color: #f77e4a;
      background: rgba(247, 126, 74, 0.1);
    }
    &.submitted {
      color: #979797;
      background: rgba(151, 151, 151, 0.1);
    }
  }

  .record-name {
    &:hover {
      cursor: pointer;
      color: var(--ant-primary-color);
    }
  }
</style>
