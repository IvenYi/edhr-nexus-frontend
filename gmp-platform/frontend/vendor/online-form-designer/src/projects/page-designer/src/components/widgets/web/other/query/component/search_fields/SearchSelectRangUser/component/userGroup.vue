...
<script setup lang="ts" name="userGroup">
  ...
  const treeDataList = computed(() => {
    ...
    if (searchVal) {
      if (props.ignoreCase) {
        return userGroupData.value?.filter(
          (e: any) => e.name?.toLocaleLowerCase().indexOf(searchVal.toLocaleLowerCase()) > -1,
        );
      } else {
        return userGroupData.value?.filter((e: any) => e.name?.indexOf(searchVal) > -1);
      }
    }
    return cloneDeep(userGroupData.value);
