...
<script lang="ts" setup name="serial-number-container">
  ...
  import { ResetConditionEnum, SerialListType, TypeEnum } from '../../../constant/serial';
  ...
  const value = computed<any>({
    get() {
      ...
      return props.serialConfigValue
        ? props.serialConfigValue
        : { modelKey: '', ruleConfig: props.increaseHidden && !props.isDesign ? [] : ruleConfig };
    },
    set(value: string[]) {
  ...
  const ruleConfigData = computed(() => {
    ...
    //   return !props.increaseHidden;
    // });
  });
  const haveIncrease = computed(() => {
    return !!ruleConfigData.value.find((i) => i.type === TypeEnum.INCREASE);
  });
  watchDebounced(
    value,
    async () => {
