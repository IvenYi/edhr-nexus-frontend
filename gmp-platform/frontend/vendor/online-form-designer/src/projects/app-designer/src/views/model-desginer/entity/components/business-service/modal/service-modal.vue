<template>
  ...
  >
    ...
    >
      ...
      <a-form-item :label="t('sys.model.operationMutualExclusion')" name="description">
        <a-switch v-model:checked="formState.mutex" />
      </a-form-item>
      <a-form-item
...
<script setup lang="ts">
  ...
  const onDataReceive = (data) => {
    ...
    const { name, key, description, method, id, serviceKey, mutex } = data;
    serviceId.value = id;
    formState.name = name;
    formState.key = key;
    // formState.key = isSYS.value ? key : keyClip(key);
    formState.method = method;
    formState.description = description;
    formState.serviceKey = serviceKey;
    formState.mutex = !!mutex;
  };
  ...
  const handleOk = async () => {
    const data = {
      ...
      ...formState,
      key: isEdit.value ? formState.key : keyPad(formState.key),
      mutex: formState.mutex ? 1 : 0,
    };
    if (isEdit.value) {
