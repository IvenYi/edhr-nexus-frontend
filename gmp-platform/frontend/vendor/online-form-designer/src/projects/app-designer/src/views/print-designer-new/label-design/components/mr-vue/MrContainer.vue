<template>
  <div
    ...
    @keydown.delete.exact.stop="emit('delete')"
...
<script lang="ts" setup name="mr-container">
  ...
  // 第三个参数可以是一个options对象，其中capture属性控制是否使用捕获
  ...
  const mouseMoveHandler = (e) => {
    ...
    if (resizing.value) {
      ...
      // 90度和270度丝滑拖动，实时进行拖动监听
      emit('resizestop', resizeStopData());
    } else if (moving.value) {
      if (sublineOn.value) {
        const el = mrElements.value[0];
        const x = offX;
        const y = offY;
        moveElementBy(el, x, y);
      } else {
      ...
      return {
        ...
        top:
          el.style.top.indexOf('%') !== -1 || el.style.top === 'auto'
            ? el.style.top
            : getTopByEl(el, parseInt(el.style.top)),
        left:
          el.style.left.indexOf('%') !== -1 || el.style.left === 'auto'
            ? el.style.left
            : getLeftByEl(el, parseInt(el.style.left)),
        bottom:
  ...
  function getTopByEl(el: Element, top) {
    top = parseInt(top);
    if (el.getAttribute('type') === PRINT_ELE_TYPE.HORIZONTAL_LINE) {
      const height = parseFloat(el.style.height) / 2 - 1;
      top += height;
    }
    return top;
  }
  function getLeftByEl(el: Element, left) {
    left = parseInt(left);
    if (el.getAttribute('type') === PRINT_ELE_TYPE.VERTICAL_LINE) {
      const width = parseFloat(el.style.width) / 2 - 1;
      left += width;
    }
    return left;
  }
  ...
  const moveElementBy = (el, offX, offY) => {
    ...
    emit('movingItem', newLeft, newTop, el.clientWidth, el.clientHeight);
  ...
  function updateListener() {
    ...
    // 先移除旧的
    elementRef.value.removeEventListener('mousedown', mouseDownHandler, {
      capture: selectedItem.value.isEdit,
    });
  ...
  watch(() => selectedItem.value.isEdit, updateListener);
</script>
