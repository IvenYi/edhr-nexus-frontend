<template>
  <div class="permission-group-setting2">
    <div v-if="!bpmnReadonly">
      <a-button size="small" type="primary" @click="addNew">
        {{ $t('sys.edhr.addPermissionGroup') }}
      </a-button>
    </div>
    <PermissionItem
      class="mt-12px"
      v-for="(item, index) in groupList"
      :key="index"
      :value="item"
      :validateItem="() => validateItems(item, index)"
      :isReadonly="bpmnReadonly"
      @change="handleChangeData"
      @delete="onDelete(index)"
      @copy="onCopy(index)"
    />
  </div>
</template>

<script lang="ts" setup name="permission-group-setting2">
  import { cloneDeep, omit } from 'lodash-es';
  import { computed, inject, ref, toRaw, watch } from 'vue';
  import { FormTmpPermissionConfig } from './type';
  import PermissionItem from './permission-item2.vue';
  import { now } from './util';

  const props = withDefaults(
    defineProps<{
      value?: FormTmpPermissionConfig[];
    }>(),
    {
      value: () => [],
    },
  );

  const bpmnReadonly = inject('bpmnReadonly', false);
  const emit = defineEmits<{
    (e: 'update:value', value: FormTmpPermissionConfig[]): void;
  }>();

  /** 权限分组列表 */
  const groupList = computed({
    get() {
      return props.value || [];
    },
    set(value) {
      emit('update:value', value);
    },
  });

  function sortList(list) {
    return list.sort((a, b) => {
      // 先比较权限等级，再比较修改时间
      if (a.permissionLevel !== b.permissionLevel) {
        return b.permissionLevel - a.permissionLevel;
      }
      return (
        (b.modifyTime ? new Date(b.modifyTime).getTime() : 0) -
        (a.modifyTime ? new Date(a.modifyTime).getTime() : 0)
      );
    });
  }

  const handleChangeData = () => {
    groupList.value = sortList(groupList.value);
    emit('update:value', groupList.value);
  };
  /** 新增权限组 */
  const addNew = () => {
    groupList.value.unshift({
      name: '',
      permissionLevel: 0,
      desc: '',
      memberPermissionSelect: 0,
      memberPermission: undefined,
      fieldPermissionSelect: 0,
      fieldPermission: undefined,
      modifyTime: now(),
      _isEditing: true,
      _isNew: true,
    });
    handleChangeData();
  };
  /** 删除权限组 */
  const onDelete = (index: number) => {
    groupList.value.splice(index, 1);
    handleChangeData();
  };

  /** 复制权限组 */
  const onCopy = (index: number) => {
    const item = cloneDeep(toRaw(groupList.value[index]));
    item.name = `${item.name}-copy`;
    item.modifyTime = now();
    groupList.value.splice(index + 1, 0, item);
    handleChangeData();
  };

  const validateItems = (item, idx) => {
    return !groupList.value.some((e, i) => e.name === item.name && i !== idx);
  };

  const getSaveData = () => {
    return toRaw(groupList.value).map((i) => omit(i, '_isEditing'));
  };

  defineExpose({ getSaveData });
</script>

<style lang="scss" scoped>
  .permission-group-setting2 {
    :deep(.ant-form-item-label > label) {
      font-size: 12px;
    }
    :deep(.ant-form label) {
      font-size: 12px;
    }
    :deep(.ant-form .ant-form-item) {
      margin-bottom: 0;
    }
  }
</style>
