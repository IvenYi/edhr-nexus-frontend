<template>
  <div class="bpmn-content-panel overflow-hidden" :class="[hidden && 'hidden']">
    <div
      class="header ks-row-middle h46px px12px"
      :class="[
        !!(DynamicPermMap[nodeSelectedData?.type] || DynamicEventMap[nodeSelectedData?.type]) &&
          'b-b',
      ]"
    >
    ...
    <div class="bpmn-panel-main" v-if="nodeSelectedId && nodeSelectedData">
      <a-tabs
        v-if="DynamicPermMap[nodeSelectedData.type] || DynamicEventMap[nodeSelectedData.type]"
...
<script setup lang="ts">
  ...
  provide('isFrontPage', !!route.query.front);
