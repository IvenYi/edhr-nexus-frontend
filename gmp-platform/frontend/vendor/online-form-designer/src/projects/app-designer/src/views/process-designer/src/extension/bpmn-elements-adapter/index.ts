...
//   // SubProcessConfig,
// } from '../bpmn/constant';
import { ElementViewSchema } from '../../constants';
...
const defaultRetainedProperties = ['properties', 'startPoint', 'endPoint', 'pointsList'];
...
let defaultTransformer: TransformerType = {
  'bpmn:startEvent': {
    out(data: any) {
      ...
      const data1 = defaultTransformer[properties.definitionType]?.out?.(data) || {};
      console.log(data1);
      return data1;
    },
  },
...
function convertNormalToXml(other?: ExtraPropsType) {
  ...
  const retainedAttrsSet = new Set([...defaultRetainedProperties, ...(retainedAttrsFields || [])]);
 ...
 */
function convertLf2ProcessData(bpmnData: any, data: any, other?: ExtraPropsType) {
  const nodeIdMap = new Map();

  const xmlJsonData = convertNormalToXml(other)(data);

  console.log('obj seq 22', xmlJsonData);

  xmlJsonData.nodes.forEach((node: any) => {
    // debugger;
    const { '-id': nodeId, '-type': nodeType, text, children, ...otherProps } = node;
    ...
    if (otherProps['-f_json']) {
      processNode['-f_json'] = otherProps['-f_json'];
    }
  ...
  bpmnData['bpmn:subProcess']?.forEach((item: any) => {
    ...
    for (let i = 0; i < edgesInSubProcess.length; ) {
      const index = sequenceFlow.findIndex((item: any) => item['-id'] === edgesInSubProcess[i]);
      if (index >= 0) {
...
function convertLf2DiagramData(bpmnDiagramData: any, data: any) {
  bpmnDiagramData['bpmndi:BPMNEdge'] = data.edges.map((edge: any) => {
    ...
    const pointsList = edge.pointsList.map(({ x, y }: { x: number; y: number }) => ({
      '-x': x,
      '-y': y,
    }));
          ...
          Object.keys(data).forEach((key: string) => {
            if (key.indexOf('bpmn:') === 0 && !ignoreType.includes(key)) {
              ...
              if (Array.isArray(data[key])) {
                ...
                !key.includes('Flow') && data['-children'].push(data[key]['-id']);
              }
              delete data[key];
            }
          });
      ...
      (key: string) => {
        if (key.indexOf('bpmn:') === 0) {
          ...
          if (key === 'bpmn:sequenceFlow') {
            const bpmnEdges =
              definitions['bpmndi:BPMNDiagram']['bpmndi:BPMNPlane']['bpmndi:BPMNEdge'];
            edges = getLfEdges(value, bpmnEdges);
          } else {
            const shapes =
              definitions['bpmndi:BPMNDiagram']['bpmndi:BPMNPlane']['bpmndi:BPMNShape'];
...
class BPMNBaseAdapter {
  ...
  constructor({ lf, pid, extensionString }: any) {
    lf.adapterIn = this.adapterIn;
    lf.adapterOut = this.adapterOut;
    this.processAttributes = {
      '-isExecutable': 'true',
      '-id': pid || 'Process',
      // '-camunda:versionTag': Date.now(),
      'bpmn:extensionElements': extensionString ?? '',
    };
...
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.START, {
  width: ElementViewSchema[BpmnElementEnum.StartEvent]?.width,
  height: ElementViewSchema[BpmnElementEnum.StartEvent]?.height,
});
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.END, {
  width: ElementViewSchema[BpmnElementEnum.EndEvent]?.width,
  height: ElementViewSchema[BpmnElementEnum.EndEvent]?.height,
});
...
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.PARALLEL_GATEWAY, {
  width: ElementViewSchema[BpmnElementEnum.ParallelGateway]?.width,
  height: ElementViewSchema[BpmnElementEnum.ParallelGateway]?.height,
});
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.INCLUSIVE_GATEWAY, {
  width: ElementViewSchema[BpmnElementEnum.InclusiveGateway]?.width,
  height: ElementViewSchema[BpmnElementEnum.InclusiveGateway]?.height,
});
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.EXCLUSIVE_GATEWAY, {
  width: ElementViewSchema[BpmnElementEnum.ExclusiveGateway]?.width,
  height: ElementViewSchema[BpmnElementEnum.ExclusiveGateway]?.height,
});
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.SYSTEM, {
  width: ElementViewSchema[BpmnElementEnum.ServiceTask]?.width,
  height: ElementViewSchema[BpmnElementEnum.ServiceTask]?.height,
});
BPMNBaseAdapter.shapeConfigMap.set(BpmnElements.USER, {
  width: ElementViewSchema[BpmnElementEnum.UserTask]?.width,
  height: ElementViewSchema[BpmnElementEnum.UserTask]?.height,
});
