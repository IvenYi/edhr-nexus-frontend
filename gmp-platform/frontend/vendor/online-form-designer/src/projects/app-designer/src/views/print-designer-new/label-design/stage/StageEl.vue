<script lang="ts">
  ...
  export default {
    ...
    render: function () {
      ...
      } else if (elementO.rotate === 270) {
        styles.transform = `rotate(${elementO.rotate}deg) translateX(-100%)`;

        styles.height = elementO.width + 'px';
        styles.width = elementO.height + 'px';

        styles.transformOrigin = `0 0`;
      } else if (elementO.rotate === 180) {
        styles.transform = `rotate(${elementO.rotate}deg)`;
      ...
      if (elementO.type === PRINT_ELE_TYPE.BAR_CODE) {
        ...
        elementO.type === PRINT_ELE_TYPE.TEXT ||
        elementO.type === PRINT_ELE_TYPE.RICH_TEXT
      ) {
        props = {
      ...
      if (elementO.type === PRINT_ELE_TYPE.QR_CODE) {
        ...
        if (elementO.rotate === 90) {
          mrElProps = { ...mrElProps, handles: ['ml', 'mr', 'lc', 'rc'] };
        } else {
          mrElProps = { ...mrElProps, handles: ['mt', 'mb', 'tc', 'bc'] };
        }
      }
      stageElem = h(
        MrEl,
