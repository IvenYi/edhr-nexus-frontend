...
<script lang="ts" setup>
  ...
  // });
  const focusCanvas = () => {
    mrContainer.value.$el.focus();
  };
  const pageStyles = computed(() => {
    return {
      ...project.value.styles,
      ...
      // 用zoom来替代transform,但只兼容IE或者chrome
      zoom: zoom.value,
      overflow: 'hidden',
    };
  });
  ...
  const arrowsHandler = ({ direction, shiftKey }) => {
    if (selectedElements.value.length > 0) {
      ...
      const parentH = parseInt(window.getComputedStyle(mrContainer.value.$el).height);
      const parentW = parseInt(window.getComputedStyle(mrContainer.value.$el).width);
  ...
  const dropHandler = (e) => {
    ...
    const maineggRect = mrContainer.value.$el.getBoundingClientRect();
