<template>
  ...
  >
    <a-form ref="formRef" :model="formState" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
      ...
      <a-form-item :label="t('sys.appDesigner.scriptKey')" name="key" :rules="[{ required: true }]">
        <a-input
          v-model:value="formState.key"
          :disabled="isEdit"
          addon-before="script_"
          show-count
          :maxlength="32"
        />
      </a-form-item>
...
<script setup lang="ts">
  ...
  const onDataReceive = (data) => {
    ...
    const prefix = 'script_';
    formState.key = key.substring(prefix.length);
    // const keyArr = key.split('_', 2);
    // formState.key = keyArr[keyArr.length - 1];
  ...
  const handleOk = async () => {
    ...
    const data = { ...formState, key: 'script_' + formState.key };
    if (isEdit.value) {
      Modal.confirm({
