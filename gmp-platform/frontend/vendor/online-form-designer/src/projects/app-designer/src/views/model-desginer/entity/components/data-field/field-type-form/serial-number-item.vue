<template>
  <div :class="['series-item-wrapper', { 'series-disable': !isShow }]">
    ...
    <div class="serial-item-container flex">
      <template v-if="!isFeild && fieldName === TypeEnum.INCREASE">
        <div
        ...
        >
          ...
          <a-form-item-rest>
            <a-input-number v-model:value="minLength" :precision="0" />
          </a-form-item-rest>
        </div>
        <div
          class="flex flex-col mr-8px"
        ...
        >
          ...
          <a-form-item-rest>
            <a-input-number v-model:value="from" :precision="0" />
          </a-form-item-rest>
        </div>
        ...
        <div class="flex flex-col mr-10px serial-num-no-error">
          ...
          <a-form-item-rest>
            <a-input-number v-model:value="to" :precision="0" />
          </a-form-item-rest>
        </div>
      ...
      <template v-else>
        <div class="serial-num-no-error flex items-end">
          <a-form-item-rest>
            <a-select
        ...
        <template v-if="configValue.type === TypeEnum.INCREASE">
          <div
            class="flex flex-col mr-8px"
            :class="{ 'serial-num-no-error': minLength != undefined && minLength != null }"
          >
        ...
        <template v-if="configValue.type === TypeEnum.PLACEHOLDER">
          <EdhrFillSelect
            ...
            :value="configValue.config.modelKey"
    ...
    <div
      v-if="isShow && (isFeild || configValue.type !== TypeEnum.INCREASE)"
      class="action"
      @click="handleDelete"
      :class="{ 'pt18px pl5px': configValue.type === TypeEnum.INCREASE }"
    >
      <a-button>
        <template #icon>
          <span class="gct-iconfont icon-icon_shanchu"></span>
        </template>
      </a-button>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  import DataFieldCascader from '../components/data-field-cascader.vue';
  import EdhrFillSelect from '../components/edhr-fill-select.vue';
  ...
  const filterTypeEnum = computed(() => {
    if (props.haveIncrease && configValue.value.type !== TypeEnum.INCREASE) {
