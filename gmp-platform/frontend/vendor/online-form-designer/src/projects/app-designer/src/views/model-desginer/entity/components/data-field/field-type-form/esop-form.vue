<template>
  <a-form-item :name="['specificConfig', 'fileSize']" :label="`${t('sys.model.limitSize')}`">
    <a-input-number
      ...
      @blur="
        () => {
          if (!formData.specificConfig.fileSize) formData.specificConfig.fileSize = 300;
        }
      "
  ...
  <a-form-item :name="['specificConfig', 'fileTypes']" :label="t('sys.model.uploadSupportType')">
    <a-select
      ...
      showArrow
    >
      <a-select-option v-for="item in uploadType.esop" :value="item.type" :key="item.type">{{
        item.type
      }}</a-select-option>
    </a-select>
  </a-form-item>
</template>
<script setup lang="ts" name="esop">
