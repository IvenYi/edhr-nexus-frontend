...
export function fixElementToParentBounds(element, parent) {
  ...
  if (top + height > parentH) {
    top -= top + height - parentH + 2;
  }
  if (left + width > parentW) {
    left -= left + width - parentW + 2;
  }
...
export function getComputedProp(prop, element, parent?) {
  if (
    ...
    return parseInt(
      window.getComputedStyle(document.getElementById(element.id).parentNode)[
        prop
      ]
    );
  } else {
    return !parent
      ? parseInt(
          window.getComputedStyle(document.getElementById(element.id))[prop]
        )
      ...
      : (parseInt(
          window.getComputedStyle(document.getElementById(parent.id))[prop]
        ) *
          parseInt(element[prop])) /
        100;
  }
}
 ...
 * @param {*} elem
 */
export function getElemCenterCoor(elem) {
  const left = elem.left;
  const right = elem.left + elem.width;
  const top = elem.top;
  const bottom = elem.top + elem.height;
