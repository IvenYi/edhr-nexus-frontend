...
<script setup lang="ts">
  ...
  const getMethodDocContent = async (methodKey: string): Promise<string | null> => {
    const modelKey = props.model?.key;
    if (!modelKey || !methodKey) {
      return null;
    }

    const docPath = `/gct-apaas/docs/methods/${encodeURIComponent(modelKey)}/${encodeURIComponent(
      methodKey,
    )}.md`;
  ...
  const handleRowDetail = async (info: ModelMethodResponse): Promise<void> => {
    if (!info.usage || info.usage.trim() === '') {
      info.usage = (await getMethodDocContent(String(info.key ?? ''))) || '';
    }
