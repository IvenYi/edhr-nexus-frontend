<template>
  <div class="zoom-menu">
    <a-tooltip class="item" effect="dark" :title="$t('sys.printDesigner.zoom.zoomIn')" placement="right">
      <button class="action-btn" @click="zoomIn" :disabled="!canZoomIn">
        <plus-outlined />
        <label></label>
      </button>
    </a-tooltip>
    <a-tooltip class="item" effect="dark" :title="$t('sys.printDesigner.zoom.zoomOut')" placement="right">
      <button class="action-btn" @click="zoomOut" :disabled="!canZoomOut">
        <minus-outlined />
      </button>
    </a-tooltip>
    <div class="zoom-value">{{ Math.round(zoom * 100) + '%' }}</div>
    <a-tooltip class="item" effect="dark" :title="$t('sys.printDesigner.zoom.recover')" placement="right">
      <button class="action-btn" @click="zoomRestore" :disabled="!canZoomIn">
        <history-outlined />
        <label></label>
      </button>
    </a-tooltip>
    <a-tooltip class="item" effect="dark" :title="$t('sys.printDesigner.zoom.fit')" placement="right">
