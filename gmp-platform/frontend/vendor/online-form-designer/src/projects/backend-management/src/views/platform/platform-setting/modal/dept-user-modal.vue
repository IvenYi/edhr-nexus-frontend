...
<script setup lang="ts" name="deptUser">
  ...
  import { getOrgList, getOrgUserPageList } from '/@/apis/gct-platform/OrgController';
  import { OrgUserResponse } from '/@/apis/gct-platform/model';
  ...
  const orgUserData = ref<OrgUserResponse[]>([]);
  ...
  const searchedOrgUserData = computed((): UserDTO[] => {
    if (searchValue.value?.trim()) {
      return orgUserData.value.filter((e: any) =>
        e.fullname?.toLocaleLowerCase().includes(searchValue.value.trim()),
      );
    }
  ...
  // 查询部门下的人员
  const getUserDataByOrg = async (orgId) => {
    const res: any = await getOrgUserPageList({
      orgId,
      pageNo: 1,
      pageSize: 999999999,
    });
    orgUserData.value = (res.data ?? []).map((e) => {
      return { ...e, formatId: `USER:${e.userId}` };
    });
  };
  ...
  const getUserInfo = async (ids) => {
    selectedUserOptions.value = (
      (await getOrgUserPickerManagementUserListByIds({ ids: ids.join(',') })) ?? []
    ).map((e) => {
