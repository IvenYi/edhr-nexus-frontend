    ...
    return [].concat(
      ...array.map((item: any) => {
        ...
        if (
          ...
          name = `${item.leftValue.key}.${item.operatorValue}:${item.key}`;
  ...
  const getDataRules = (type, mainModelKey) => {
    ...
    list.forEach((item: any) => {
      if (item.operatorType === 'content') {
        ...
        } else key = `${item.leftValue.key}.${item.operatorValue}:${item.key}`;
