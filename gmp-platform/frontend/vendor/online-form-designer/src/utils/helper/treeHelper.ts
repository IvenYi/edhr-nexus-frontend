...
export function listToTree<T = any>(
  ...
  for (const node of list) {
    if (!deep) {
      node[children] = node[pid] === 'ROOT' ? node[children] || [] : [];
    } else {
      node[children] = node[children] || undefined;
    }

    nodeMap.set(node[id], callBack(node));
  }
  for (const node of list) {
    ...
    // console.log(parent);
    (parent ? parent[children] : result).push(callBack(node));
  }
  return result;
}
 ...
 *
 * @param tree 树
 * @param startId 从那个节点开始向上寻找path
 * @param func 需要验证成立的条件函数
 * @param config 设置树的字段
 * @returns
 */
export function findClosestParent(
  ...
  config = getConfig(config);
  const { id } = config;
  const pathNodeList: Array<any> = findPath(
    tree,
    (node) => {
      return startId == node[id!];
    },
    config,
  );
  const parent = pathNodeList?.reverse().find((d) => {
    return func(d);
  });
  return parent;
}
...
export function traverseAndBuildTree(
  treeData: any[],
  callback: Function,
  config: Partial<TreeHelperConfig> = {},
) {
  ...
  return treeData.map((node) => {
    ...
    if (children && node[children] && node[children].length > 0) {
      newNode[children] = traverseAndBuildTree(node.children, callback);
    }
...
export function list_to_tree(list: any[], callBack = (n) => n) {
  const map = {},
    roots: any[] = [];
  for (let i = 0; i < list.length; i++) {
    map[list[i].id] = i;
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (node.parentId === 'ROOT' || !node.parentId) {
      ...
      children && children.push(callBack(node));
    }
  }
  return roots;
}
