...
<script setup lang="ts" name="HandwritingSigner">
  ...
  import { SignerExpose, UploadSignFile } from '../types';
  import { completeSignInfo, uploadSignFile as defaultFn } from '../logic';
  import { useNamespace } from '@gct/runtime';
  import { message } from 'ant-design-vue';

  const ns = useNamespace('handwriting-signer');
  ...
  defineExpose<SignerExpose>({
    submit: async () => {
      ...
      if (!value) {
        message.error(t('sys.inputTextTip', { name: t('sys.model.sign') }));
...
<style lang="scss" scoped>
  @include b(handwriting-signer) {
    width: 540px;
    @include e(wacom-wrap) {
      width: 539px;
      height: 306px;
