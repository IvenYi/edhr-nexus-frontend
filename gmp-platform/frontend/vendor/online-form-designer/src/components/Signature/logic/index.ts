...
import { GetSignImgByAccount, PartialSignInfo, SignInfo, UploadSignFile } from '../types';
...
export async function uploadSignFile(file: File, modelKey?: string) {
  const url = await Uploader.uploadByFile(file, true, modelKey);
  return { url };
}
 ...
 */
export async function openSignModal(
  opts: {
  ...
  const res = await gct.openUtil.modal(
    ...
    {
      title: $t('sys.pageDesigner.handwrittenSignature'),
      width: 640,
      height: 600,
      okText: $t('sys.okText'),
      showFooter: true,
 ...
 * @return {*}
 */
export function getPreviewUrl(url, name, callback?: any) {
  if (url) {
    if (callback && typeof callback === 'function') {
      return callback?.(url);
    }
    return `${import.meta.env.VITE_MINIO_PATH}${!url.startsWith('/') ? `/${url}` : url}`;
  } else if (name) {
