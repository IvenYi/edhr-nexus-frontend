<template>
  <div :class="[prefixCls]">
      ...
      <ScrollContainer>
        <ul>
          ...
          >
            ...
            >
              <template #content>
                <div class="flex items-center h-36px pl-10px pr-10px primary-gct font-500">{{
                  t(menu.name)
                }}</div>
    ...
    >
      <div v-if="isSearchable" style="margin-right: 10px" class="my16px px8px">
        <a-input
...
<script setup lang="ts" name="menu-default">
  ...
  const prefixCls = 'menu-mix-sider';
...
<style lang="less">
  @import url('./menu-mix-sider.less');
</style>
