<template>
  <a-form-item :label="t('受控组件')" name="widgetId">
    <a-select
      ...
      @select="handleSelect"
    >
      <a-select-opt-group v-for="group in forms" :key="group.id">
        <template #label>
  ...
  <a-form-item :label="t('执行动作')" name="action">
    ...
    >
      <a-select-option
        v-for="item in widgetActions[FormComponents.Form]"
        :key="item.value"
        :value="item.value"
      >
        {{ item.label }}
...
<script lang="ts" setup>
  ...
  const { t } = useI18n();
  const { forms, widgetIdMap } = useWidgetPicker();

  const props = defineProps({
    formState: {
      type: Object as PropType<Control.WidgetForm>,
      default: () => ({}),
    },
  });
  ...
  const dynamicProps = computed(() => {
    const action = widgetActions[FormComponents.Form].find(
