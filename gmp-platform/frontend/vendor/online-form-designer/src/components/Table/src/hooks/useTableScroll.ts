...
export function useTableScroll(
  ...
  const tableHeightRef: Ref<Nullable<number | string>> = ref(215);
  ...
  async function calcTableHeight() {
    ...
    setHeight(height);
    if (propsRef.value.pagination) {
      tableEl.querySelector('.ant-table-body')!.style.height = `${height}px`;
    }
  }
  ...
  const getScrollX = computed(() => {
    ...
    return tableWidth > width ? undefined : width;
  });
