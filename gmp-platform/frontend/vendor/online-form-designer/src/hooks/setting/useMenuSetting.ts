...
export function useMenuSetting() {
  ...
  const getMenuType = computed(() => {
    if (appStore.getThemeSetting.toolbarMode === 'left') {
      return MenuTypeEnum.SIDEBAR;
    }
    if (appStore.getThemeSetting.toolbarMode === 'top') {
      return MenuTypeEnum.TOP_MENU;
    }
    return MenuTypeEnum.MIX;
  });
  ...
  const getMenuWidth = computed(
    () => appStore.getThemeSetting.width || appStore.getMenuSetting.menuWidth,
  );

  // 配置折叠面板
  const getTrigger = computed(() => {
    if (!appStore.getThemeSetting.menuSetting.includes('flod')) {
      return TriggerEnum.NONE;
    }
    if (appStore.getThemeSetting.menuSettingFold === 'bottom') {
      return TriggerEnum.FOOTER;
    }
    return TriggerEnum.HEADER;
  });
  ...
  // const getAccordion = computed(() => appStore.getThemeSetting.menuSetting.includes('accordion'));
  const getAccordion = false;

  const getMixSideFixed = computed(() => appStore.getMenuSetting.mixSideFixed);

  // 顶部菜单布局appStore.getMenuSetting.topMenuAlign
  const getTopMenuAlign = computed(() => {
    if (appStore.getThemeSetting.menuTopLayout.includes('top')) {
      return 'start';
    }
    if (appStore.getThemeSetting.menuTopLayout.includes('bottom')) {
      return 'end';
    }
    return 'center';
  });
